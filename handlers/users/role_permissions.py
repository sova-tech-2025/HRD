...
from keyboards.keyboards import (
    get_permission_action_keyboard,
    get_permission_confirmation_keyboard,
    get_permission_selection_keyboard,
...
async def show_roles_list(message: Message, state: FSMContext, session: AsyncSession):
    ...
    await message.answer("Выбери роль для управления правами:", reply_markup=keyboard)
...
{perms_display}"""
    ...
    await callback.message.edit_text(role_info, reply_markup=keyboard, parse_mode="HTML")
...
async def process_add_permission(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    await callback.message.edit_text("Выбери возможность, которую хочешь добавить этой роли:", reply_markup=keyboard)
    ...
    log_user_action(
        callback.from_user.id, callback.from_user.username, "opened add permission menu", {"role_id": role_id}
    )
...
async def process_remove_permission(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    await callback.message.edit_text("Выбери возможность, которую хочешь убрать у этой роли:", reply_markup=keyboard)
    ...
    log_user_action(
        callback.from_user.id, callback.from_user.username, "opened remove permission menu", {"role_id": role_id}
    )
...
async def process_permission_selection(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    confirmation_text = (
        f"Ты действительно хочешь {action_text} возможность '{permission.description}' для роли '{role.name}'?"
    )
    ...
    log_user_action(
        ...
        {"role_id": role.id, "role_name": role.name, "permission": permission.name},
    )
...
async def process_permission_confirmation(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    if success:
        ...
        log_user_action(
            ...
            callback.from_user.username,
            log_msg,
            {"role_id": role_id, "role_name": role_name, "permission": permission_name},
        )
    else:
...
{perms_display}"""
    ...
    log_user_action(
        callback.from_user.id, callback.from_user.username, "cancelled permission selection", {"role_id": role.id}
    )
...
async def process_cancel_role_management(callback: CallbackQuery, state: FSMContext):
    ...
    log_user_action(callback.from_user.id, callback.from_user.username, "cancelled role management")
...
async def process_cancel_permission_confirmation(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    if action == "add":
        ...
        if available_permissions:
            keyboard = get_permission_selection_keyboard(available_permissions, role_id, "add")
            await callback.message.edit_text(
                "Выбери возможность, которую хочешь добавить этой роли:", reply_markup=keyboard
            )
        else:
            await callback.message.answer("Все доступные возможности уже добавлены этой роли.")
        ...
        if current_permissions:
            keyboard = get_permission_selection_keyboard(current_permissions, role_id, "remove")
            await callback.message.edit_text(
                "Выбери возможность, которую хочешь убрать у этой роли:", reply_markup=keyboard
            )
        else:
            await callback.message.answer("У этой роли нет возможностей, которые можно ограничить.")
            process_cancel_permission_selection(callback, state, session)
...
async def process_cancel_role_change(callback: CallbackQuery, state: FSMContext):
    ...
    log_user_action(callback.from_user.id, callback.from_user.username, "cancelled role change")
