...
class User(Base):
    ...
    assigned_learning_paths = relationship(
        "TraineeLearningPath", foreign_keys="TraineeLearningPath.trainee_id", back_populates="trainee"
    )
...
class Test(Base):
    ...
    sessions = relationship(
        "LearningSession", secondary=session_tests, back_populates="tests"
    )  # Связь с сессиями траекторий
    company = relationship("Company")
...
class LearningPath(Base):
    ...
    assigned_trainees = relationship(
        "TraineeLearningPath", back_populates="learning_path", cascade="all, delete-orphan"
    )
    company = relationship("Company")
...
class LearningStage(Base):
    ...
    sessions = relationship(
        "LearningSession", back_populates="stage", cascade="all, delete-orphan", order_by="LearningSession.order_number"
    )
...
class LearningSession(Base):
    ...
    tests = relationship(
        "Test", secondary=session_tests, back_populates="sessions", order_by=session_tests.c.order_number
    )
...
class Attestation(Base):
    ...
    questions = relationship(
        ...
        cascade="all, delete-orphan",
        order_by="AttestationQuestion.question_number",
    )
...
class AttestationQuestion(Base):
    ...
    def __repr__(self):
        return (
            f"<AttestationQuestion(id={self.id}, attestation_id={self.attestation_id}, number={self.question_number})>"
        )
...
class TraineeStageProgress(Base):
    ...
    session_progress = relationship(
        "TraineeSessionProgress", back_populates="stage_progress", cascade="all, delete-orphan"
    )
...
class KnowledgeFolder(Base):
    ...
    materials = relationship(
        ...
        cascade="all, delete-orphan",
        order_by="KnowledgeMaterial.order_number",
    )
