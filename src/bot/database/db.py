...
async def ensure_company_id(session: AsyncSession, state: FSMContext, user_id: int) -> Optional[int]:
    """
    ...
    async with async_session() as session:
        ...
        if count == 0:
                ...
                if role.name != "Руководитель":
                    for perm in permissions:
                        if perm.name in ["view_profile", "edit_profile"]:
                            stmt = insert(role_permissions).values(role_id=role.id, permission_id=perm.id)
            ...
            stages = [
                ...
                InternshipStage(
                    name="Практическое применение", description="Работа с реальными задачами", order_number=3
                ),
    ...
    async with async_session() as session:
        try:
            ...
            if not mentor_role:
                ...
                for perm in permissions:
                    if perm.name in [
                        ...
                        stmt = insert(role_permissions).values(role_id=mentor_role.id, permission_id=perm.id)
                        await session.execute(stmt)
            ...
            if mentor_role_created:
                ...
                if employee_role:
                    ...
                    for user in users:
                        ...
                        await session.execute(insert(user_roles).values(user_id=user.id, role_id=mentor_role.id))

                        logger.info(
                            f"Пользователь {user.full_name} (ID: {user.id}) переведен с роли 'Сотрудник' на 'Наставник'"
                        )
            ...
            if employee_role:
                ...
                await session.execute(delete(role_permissions).where(role_permissions.c.role_id == employee_role.id))
                ...
                for perm in permissions:
                    if perm.name in ["view_profile", "edit_profile", "take_tests", "view_test_results"]:
                        stmt = insert(role_permissions).values(role_id=employee_role.id, permission_id=perm.id)
                        await session.execute(stmt)
    ...
    async with async_session() as session:
        try:
            ...
            result = await session.execute(
                update(User).where(User.role_assigned_date.is_(None)).values(role_assigned_date=User.registration_date)
            )
...
async def check_phone_exists(session: AsyncSession, phone_number: str) -> bool:
    result = await session.execute(select(func.count()).select_from(User).where(User.phone_number == phone_number))
    count = result.scalar()
    return count > 0
...
async def get_user_roles(session: AsyncSession, user_id: int) -> List[Role]:
    try:
        stmt = select(Role).join(user_roles, Role.id == user_roles.c.role_id).where(user_roles.c.user_id == user_id)
...
async def add_user_role(session: AsyncSession, user_id: int, role_name: str) -> bool:
    try:
        ...
        check_stmt = (
            ...
            .where(user_roles.c.user_id == user_id, user_roles.c.role_id == role.id)
        )
...
async def remove_user_role(session: AsyncSession, user_id: int, role_name: str) -> bool:
    try:
        ...
        stmt = delete(user_roles).where(user_roles.c.user_id == user_id, user_roles.c.role_id == role.id)
        await session.execute(stmt)
...
async def activate_user(
    ...
    try:
        ...
        if bot:
            await send_notification_about_activation(
                session, bot, user_id, role_name, group_id, internship_object_id, work_object_id, final_company_id
            )
...
async def add_permission_to_role(session: AsyncSession, role_id: int, permission_name: str) -> bool:
    ...
    try:
        perm_result = await session.execute(select(Permission).where(Permission.name == permission_name))
        ...
        check_stmt = (
            ...
            .where(role_permissions.c.role_id == role_id, role_permissions.c.permission_id == permission.id)
        )
        check_result = await session.execute(check_stmt)
        ...
        stmt = insert(role_permissions).values(role_id=role_id, permission_id=permission.id)
        await session.execute(stmt)
...
async def remove_permission_from_role(session: AsyncSession, role_id: int, permission_name: str) -> bool:
    ...
    try:
        ...
        stmt = delete(role_permissions).where(
            role_permissions.c.role_id == role_id, role_permissions.c.permission_id == permission.id
        )
        await session.execute(stmt)
...
async def create_group(session: AsyncSession, name: str, created_by_id: int, company_id: int = None) -> Optional[Group]:
    ...
    try:
        ...
        group = Group(name=name, created_by_id=created_by_id, company_id=company_id)
        session.add(group)
        await session.commit()
...
async def get_all_groups(session: AsyncSession, company_id: int = None) -> List[Group]:
    ...
    try:
        ...
        query = (
            select(Group)
            .options(selectinload(Group.users).selectinload(User.roles))
            .where(Group.is_active == True, Group.company_id == company_id)
            .order_by(Group.name)
        )
...
async def update_group_name(session: AsyncSession, group_id: int, new_name: str, company_id: int = None) -> bool:
    ...
    try:
        ...
        check_stmt = (
            select(func.count())
            .select_from(Group)
            .where(Group.name == new_name, Group.id != group_id, Group.is_active == True)
        )
...
async def get_all_users_in_group(session: AsyncSession, group_id: int, company_id: int = None) -> List[User]:
    ...
    try:
        ...
        stmt = (
            ...
            .where(user_groups.c.group_id == group_id, User.is_activated == True, User.is_active == True)
        )
...
async def get_user_groups(session: AsyncSession, user_id: int, company_id: int = None) -> List[Group]:
    ...
    try:
        ...
        stmt = (
            ...
            .where(user_groups.c.user_id == user_id, Group.is_active == True)
        )
...
async def add_user_to_group(session: AsyncSession, user_id: int, group_id: int, company_id: int = None) -> bool:
    ...
    try:
        ...
        if company_id is not None and user.company_id != company_id:
            logger.error(
                f"Попытка добавить пользователя {user_id} из компании {user.company_id} в группу компании {company_id}"
            )
            return False
        ...
        check_stmt = (
            ...
            .where(user_groups.c.user_id == user_id, user_groups.c.group_id == group_id)
        )
        check_result = await session.execute(check_stmt)
        ...
        stmt = insert(user_groups).values(user_id=user_id, group_id=group_id)
        await session.execute(stmt)
        await session.commit()
...
async def remove_user_from_group(session: AsyncSession, user_id: int, group_id: int, company_id: int = None) -> bool:
    ...
    try:
        ...
        if company_id is not None and user.company_id != company_id:
            logger.error(
                f"Попытка удалить пользователя {user_id} из компании {user.company_id} из группы компании {company_id}"
            )
            return False
        ...
        stmt = delete(user_groups).where(user_groups.c.user_id == user_id, user_groups.c.group_id == group_id)
        await session.execute(stmt)
        await session.commit()
...
async def create_object(
    session: AsyncSession, name: str, created_by_id: int, company_id: int = None
) -> Optional[Object]:
    ...
    try:
        ...
        new_object = Object(name=name.strip(), created_by_id=created_by_id, company_id=company_id)
...
async def update_object_name(session: AsyncSession, object_id: int, new_name: str, company_id: int = None) -> bool:
    ...
    try:
        ...
        check_query = select(Object).where(
            Object.name == new_name.strip(), Object.id != object_id, Object.is_active == True
        )
...
async def get_tests_by_creator(session: AsyncSession, creator_id: int, company_id: int = None) -> List[Test]:
    ...
    try:
        ...
        result = await session.execute(query.order_by(Test.created_date.desc()))
        return result.scalars().all()
    except Exception as e:
...
async def get_all_active_tests(session: AsyncSession, company_id: int = None) -> List[Test]:
    ...
    try:
        ...
        query = (
            select(Test).where(Test.is_active == True, Test.company_id == company_id).order_by(Test.created_date.desc())
        )
...
async def add_question_to_test(
    session: AsyncSession, question_data: dict, company_id: int = None
    ...
    try:
        ...
        question = TestQuestion(
            ...
            if isinstance(question_data["correct_answer"], list)
            else question_data["correct_answer"],
...
async def get_all_stages(session: AsyncSession) -> List[InternshipStage]:
    ...
    try:
        result = await session.execute(
            select(InternshipStage).where(InternshipStage.is_active == True).order_by(InternshipStage.order_number)
        )
        return result.scalars().all()
    except Exception as e:
...
async def assign_mentor(
    session: AsyncSession, mentor_id: int, trainee_id: int, assigned_by_id: int, bot=None, company_id: int = None
) -> Optional[Mentorship]:
    ...
    try:
        ...
        existing = await session.execute(
            select(Mentorship).where(Mentorship.trainee_id == trainee_id, Mentorship.is_active == True)
        )
        ...
        if existing_mentorship:
            ...
            await session.execute(
                update(Mentorship)
                .where(Mentorship.trainee_id == trainee_id, Mentorship.is_active == True)
                .values(is_active=False)
            )

        mentorship = Mentorship(
            mentor_id=mentor_id, trainee_id=trainee_id, assigned_by_id=assigned_by_id, company_id=company_id
        )
        ...
        if bot:
            await send_notification_about_new_trainee(session, bot, mentor_id, trainee_id, assigned_by_id, company_id)
...
async def get_mentor_trainees(session: AsyncSession, mentor_id: int, company_id: int = None) -> List[User]:
    ...
    try:
        ...
        if company_id is not None:
            query = query.where(User.company_id == company_id, Mentorship.company_id == company_id)
...
async def grant_test_access(
    session: AsyncSession, trainee_id: int, test_id: int, granted_by_id: int, company_id: int = None, bot=None
) -> bool:
    ...
    try:
        ...
        if not existing_access:
            ...
            logger.info(
                f"Доступ к тесту {test_id} для стажёра {trainee_id} уже существует - отправляем повторное уведомление"
            )
...
async def get_trainee_available_tests(session: AsyncSession, trainee_id: int, company_id: int = None) -> List[Test]:
    ...
    try:
        ...
        if company_id is not None:
            from bot.database.models import LearningPath

            trainee_path_query = trainee_path_query.join(
                LearningPath, TraineeLearningPath.learning_path_id == LearningPath.id
            ).where(User.company_id == company_id, LearningPath.company_id == company_id)
        ...
        result = await session.execute(query.order_by(Test.created_date))
        return result.scalars().all()
    except Exception as e:
...
async def get_user_available_tests(
    session: AsyncSession, user_id: int, exclude_completed: bool = True, company_id: int = None
) -> List[Test]:
    """
...
async def get_user_broadcast_tests(
    session: AsyncSession, user_id: int, exclude_completed: bool = False, company_id: int = None
) -> List[Test]:
    """
    ...
    try:
        ...
        query_tests = (
            ...
            .where(TraineeTestAccess.trainee_id == user_id, TraineeTestAccess.is_active == True, Test.is_active == True)
        )

        # Изоляция по компании
        if company_id is not None:
            query_tests = query_tests.where(Test.company_id == company_id, User.company_id == company_id)
        ...
        if trainee_path:
            ...
            trajectory_tests_result = await session.execute(
                ...
                .join(TraineeSessionProgress, TraineeSessionProgress.session_id == LearningSession.id)
                .join(TraineeStageProgress, TraineeSessionProgress.stage_progress_id == TraineeStageProgress.id)
                .where(TraineeStageProgress.trainee_path_id == trainee_path.id, TraineeStageProgress.is_opened == True)
            )
...
async def get_employee_tests_from_recruiter(
    session: AsyncSession, user_id: int, exclude_completed: bool = True, company_id: int = None
) -> List[Test]:
    """
...
async def revoke_test_access(session: AsyncSession, trainee_id: int, test_id: int, company_id: int = None) -> bool:
    ...
    try:
        ...
        if company_id is not None:
            ...
            if not access:
                logger.error(
                    f"Доступ к тесту {test_id} для стажера {trainee_id} не найден или не принадлежит компании {company_id}"
                )
                return False

        stmt = update(TraineeTestAccess).where(
            TraineeTestAccess.trainee_id == trainee_id, TraineeTestAccess.test_id == test_id
        )
...
async def get_user_test_results(session: AsyncSession, user_id: int, company_id: int = None) -> List[TestResult]:
    ...
    try:
        ...
        result = await session.execute(query.order_by(TestResult.created_date.desc()))
        return result.scalars().all()
    except Exception as e:
...
async def get_test_results_summary(session: AsyncSession, test_id: int, company_id: int = None) -> List[TestResult]:
    ...
    try:
        query = select(TestResult).join(Test, TestResult.test_id == Test.id).where(TestResult.test_id == test_id)

        # КРИТИЧЕСКАЯ ИЗОЛЯЦИЯ ПО КОМПАНИИ!
        if company_id is not None:
            query = query.where(Test.company_id == company_id)

        result = await session.execute(query.order_by(TestResult.score.desc()))
        return result.scalars().all()
    except Exception as e:
...
async def delete_trajectory_test_results(
    session: AsyncSession, trainee_id: int, learning_path_id: int, company_id: int = None
) -> bool:
    """Удаление результатов тестов при повторном назначении траектории (с изоляцией по компании)"""
    try:
        from bot.database.models import LearningPath, LearningSession, LearningStage, session_tests
        ...
        deleted_count = await session.execute(
            delete(TestResult).where(TestResult.user_id == trainee_id, TestResult.test_id.in_(test_ids))
        )

        await session.commit()
        logger.info(
            f"Удалено {deleted_count.rowcount} результатов тестов для стажера {trainee_id} в траектории {learning_path_id}"
        )
        return True
        ...
        await session.rollback()
        logger.error(
            f"Ошибка удаления результатов тестов для стажера {trainee_id} в траектории {learning_path_id}: {e}"
        )
        return False
...
async def check_test_already_passed(session: AsyncSession, user_id: int, test_id: int) -> bool:
    ...
    try:
        result = await session.execute(
            ...
            .where(TestResult.user_id == user_id, TestResult.test_id == test_id)
        )
        count = result.scalar()
        return count > 0
    except Exception as e:
...
async def check_test_access(session: AsyncSession, user_id: int, test_id: int, company_id: int = None) -> bool:
    ...
    try:
        ...
        if "Стажер" in role_names:
            if company_id is None:
                logger.warning(
                    f"Не удалось определить company_id для проверки доступа пользователя {user_id} к тесту {test_id}"
                )
                return False
            ...
            query = select(TraineeTestAccess).where(
                ...
                TraineeTestAccess.test_id == test_id,
                TraineeTestAccess.is_active == True,
                (TraineeTestAccess.company_id == company_id) | (TraineeTestAccess.company_id.is_(None)),
            )
...
async def get_user_test_result(
    session: AsyncSession, user_id: int, test_id: int, company_id: int = None
) -> Optional[TestResult]:
...
async def get_user_test_attempts_count(
    session: AsyncSession, user_id: int, test_id: int, company_id: int = None
) -> int:
...
async def send_notification_about_new_test(
    session: AsyncSession, bot, trainee_id: int, test_id: int, granted_by_id: int, company_id: int = None
):
...
async def send_notification_about_mentor_assignment(
    session: AsyncSession, bot, trainee_id: int, mentor_id: int, assigned_by_id: int, company_id: int = None
):
...
Можно открыть его из раздела «Мои тесты» и начать, когда тебе будет удобно."""
        ...
        await bot.send_message(chat_id=trainee_tg_id, text=notification_text, parse_mode="HTML", reply_markup=keyboard)
...
async def send_broadcast_notification(
    ...
    broadcast_photos: list,
    broadcast_material_id: int = None,
    test_id: int = None,
    broadcast_docs: list | None = None,
) -> bool:
    """
    ...
    """
    try:
        from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup, InputMediaDocument, InputMediaPhoto
        ...
        if broadcast_material_id:
            keyboard.append(
                [InlineKeyboardButton(text="Материалы 📚", callback_data=f"broadcast_material:{broadcast_material_id}")]
            )
...
async def send_mentor_assignment_notification(
    ...
    mentor_name: str,
    mentor_phone: str,
    mentor_username: str = None,
    assigned_by_name: str = None,
    trainee_internship_object: str = None,
    trainee_work_object: str = None,
    mentor_work_object: str = None,
    company_id: int = None,
):
    """Отправка уведомления стажеру о назначении наставника (с изоляцией по компании)"""
    try:
...
async def send_notification_about_new_trainee(
    session: AsyncSession, bot, mentor_id: int, trainee_id: int, assigned_by_id: int, company_id: int = None
):
...
async def send_trainee_assignment_notification(
    bot,
    mentor_tg_id: int,
    trainee_name: str,
    trainee_phone: str,
    trainee_tg_id: int = None,
    trainee_username: str = None,
    trainee_registration_date: str = None,
    assigned_by_name: str = None,
    trainee_roles: list = None,
    trainee_groups: list = None,
    trainee_number: int = None,
    trainee_internship_object: str = None,
    trainee_work_object: str = None,
):
...
Теперь свяжись со стажером, согласуй план обучение, выдай доступ к тестам, помогай и отслеживай прогресс. Успехов в наставничестве!"""
        ...
        if trainee_tg_id:
            keyboard_buttons.append(
                [InlineKeyboardButton(text="💬 Написать стажёру", url=f"tg://user?id={trainee_tg_id}")]
            )
        elif trainee_username:
            keyboard_buttons.append(
                [InlineKeyboardButton(text="💬 Написать стажёру", url=f"https://t.me/{trainee_username}")]
            )
        ...
        await bot.send_message(chat_id=mentor_tg_id, text=notification_text, parse_mode="HTML", reply_markup=keyboard)
...
async def send_notification_about_new_trainee_registration(session: AsyncSession, bot, trainee_id: int):
    ...
    try:
        ...
        logger.info(
            f"Уведомления о новом стажёре отправлены {success_count}/{len(recruiters)} рекрутерам компании {trainee.company_id}"
        )
...
async def send_new_trainee_registration_notification(
    bot,
    recruiter_tg_id: int,
    trainee_name: str,
    trainee_phone: str,
...
⚡ <b>Быстрые действия:</b>"""
        ...
        await bot.send_message(
            chat_id=recruiter_tg_id, text=notification_text, parse_mode="HTML", reply_markup=keyboard
        )
...
async def send_notification_about_activation(
    session: AsyncSession,
    bot,
    user_id: int,
    role_name: str,
    ...
    try:
        ...
        if internship_object_id:
            ...
            if not internship_object:
                logger.error(
                    f"Объект стажировки {internship_object_id} не найден или не принадлежит компании {company_id}"
                )
                return False
        ...
        details_lines = [f"Твоя роль: <b>{role_name}</b>", f"Твоя группа: <b>{group_name}</b>"]
        ...
        notification_text = "✅ Доступ активирован\nДобро пожаловать в команду!\n_______\n\n" + "\n".join(details_lines)
...
async def send_notification_about_new_user_registration(
    session: AsyncSession, bot, user_id: int, company_id: int = None
):
    ...
    try:
        ...
        registration_date = (
            user.registration_date.strftime("%d.%m.%Y %H:%M") if user.registration_date else "Не указана"
        )
...
⚠️<b>Требует активации!</b> Используй список "Новые пользователи" для назначения роли, группы и объектов"""
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="Новые пользователи", callback_data="show_new_users")]]
        )
        ...
        for recruiter in recruiters:
            try:
                await bot.send_message(
                    chat_id=recruiter.tg_id, text=notification_text, parse_mode="HTML", reply_markup=keyboard
                )
...
async def get_users_by_group(session: AsyncSession, group_id: int, company_id: int = None) -> List[User]:
    ...
    try:
        ...
        result = await session.execute(query_filter.order_by(User.full_name))
        return result.scalars().all()
    except Exception as e:
...
async def search_unactivated_users_by_name(session: AsyncSession, query: str, company_id: int = None) -> List[User]:
    ...
    try:
        ...
        result = await session.execute(query_filter.order_by(User.registration_date.desc()))
        users = result.scalars().all()
...
async def update_user_full_name(
    session: AsyncSession, user_id: int, new_full_name: str, recruiter_id: int, bot=None, company_id: int = None
) -> bool:
    ...
    try:
        ...
        if bot:
            await send_notification_about_data_change(
                session, bot, user_id, recruiter_id, "ФИО", old_full_name, new_full_name, company_id
            )
...
async def update_user_phone_number(
    session: AsyncSession, user_id: int, new_phone_number: str, recruiter_id: int, bot=None, company_id: int = None
) -> bool:
    ...
    try:
        ...
        if bot:
            await send_notification_about_data_change(
                session, bot, user_id, recruiter_id, "ТЕЛЕФОН", old_phone, new_phone_number, company_id
            )
...
async def update_user_role(
    session: AsyncSession, user_id: int, new_role_name: str, recruiter_id: int, bot=None, company_id: int = None
) -> bool:
    """Безопасное обновление роли пользователя с очисткой связанных данных (с изоляцией по компании)"""
    try:
        from bot.database.models import Mentorship, TraineeAttestation, TraineeLearningPath, TraineeTestAccess
        ...
        # Получаем новую роль
        role_result = await session.execute(select(Role).where(Role.name == new_role_name))
        ...
        if old_role_name == "Стажер":
            ...
            mentorship_query = update(Mentorship).where(Mentorship.trainee_id == user_id, Mentorship.is_active == True)

            # Изоляция по компании - КРИТИЧЕСКИ ВАЖНО!
            if company_id is not None:
                # Получаем наставничество через join с User для проверки company_id
                mentorship_query = mentorship_query.where(
                    Mentorship.mentor_id.in_(select(User.id).where(User.company_id == company_id))
                )
            ...
            test_access_query = update(TraineeTestAccess).where(
                TraineeTestAccess.trainee_id == user_id, TraineeTestAccess.is_active == True
            )
            ...
            trainee_attestation_update_query = update(TraineeAttestation).where(
                TraineeAttestation.trainee_id == user_id, TraineeAttestation.is_active == True
            )
            ...
            logger.info("Очищены данные стажера: траектории, наставничество, доступ к тестам, аттестации")
        ...
        if old_role_name == "Наставник":
            ...
            mentorship_query = select(Mentorship).where(Mentorship.mentor_id == user_id, Mentorship.is_active == True)

            # Изоляция по компании - КРИТИЧЕСКИ ВАЖНО!
            if company_id is not None:
                mentorship_query = mentorship_query.join(User, Mentorship.trainee_id == User.id).where(
                    User.company_id == company_id
                )
            ...
            if active_mentorships:
                ...
                mentorship_update_query = update(Mentorship).where(
                    Mentorship.mentor_id == user_id, Mentorship.is_active == True
                )

                # Изоляция по компании - КРИТИЧЕСКИ ВАЖНО!
                if company_id is not None:
                    mentorship_update_query = mentorship_update_query.where(
                        Mentorship.trainee_id.in_(select(User.id).where(User.company_id == company_id))
                    )
        ...
        if new_role_name == "Стажер":
            logger.info(
                f"Пользователь {user_id} становится стажером - очистка старых данных и подготовка к назначению наставника"
            )
            ...
            deleted_results = await session.execute(delete(TestResult).where(TestResult.user_id == user_id))
        ...
        if bot:
            ...
            await send_notification_about_data_change(
                session, bot, user_id, recruiter_id, "РОЛЬ", old_role_name, notification_text, company_id
            )
...
async def get_role_change_warnings(
    session: AsyncSession, user_id: int, old_role: str, new_role: str, company_id: int = None
) -> str:
    """Получение предупреждений о последствиях смены роли (с изоляцией по компании)"""
    try:
        from bot.database.models import Mentorship, TraineeLearningPath, TraineeTestAccess
        ...
        if old_role == "Стажер":
            ...
            trainee_path_result = await session.execute(
                select(TraineeLearningPath).where(
                    TraineeLearningPath.trainee_id == user_id, TraineeLearningPath.is_active == True
                )
            )
            ...
            mentorship_query = select(Mentorship).where(Mentorship.trainee_id == user_id, Mentorship.is_active == True)
            ...
            if active_mentorship:
                ...
                warnings.append(
                    f"❌<b>ПОТЕРЯ НАСТАВНИКА:</b> Связь с наставником {mentor.full_name} будет деактивирована"
                )
            ...
            if active_test_access:
                warnings.append(
                    f"❌<b>ПОТЕРЯ ДОСТУПА:</b> Доступ к {len(active_test_access)} тестам стажера будет отозван"
                )
            ...
            if active_attestations:
                warnings.append(
                    f"❌<b>ПОТЕРЯ АТТЕСТАЦИЙ:</b> {len(active_attestations)} назначенных аттестаций будут деактивированы"
                )
...
async def cleanup_all_duplicate_attestations(session: AsyncSession, company_id: int = None) -> dict:
    ...
    try:
        ...
        cleanup_report = {"users_processed": 0, "duplicates_found": 0, "duplicates_removed": 0, "affected_users": []}
...
async def verify_role_system_integrity(session: AsyncSession, company_id: int = None) -> dict:
    ...
    try:
        ...
        for trainee in all_trainees:
            query_check = select(Mentorship).where(Mentorship.trainee_id == trainee.id, Mentorship.is_active == True)
            if company_id is not None:
            ...
            if not mentorship_check.scalar_one_or_none():
                integrity_report["trainees_without_mentors"].append(
                    {"id": trainee.id, "name": trainee.full_name, "tg_id": trainee.tg_id}
                )
        ...
        for mentor in all_mentors:
            query_check = select(Mentorship).where(Mentorship.mentor_id == mentor.id, Mentorship.is_active == True)
            if company_id is not None:
                query_check = query_check.where(Mentorship.company_id == company_id)
            ...
            if not mentorship_check.scalars().all():
                integrity_report["mentors_without_trainees"].append(
                    {"id": mentor.id, "name": mentor.full_name, "tg_id": mentor.tg_id}
                )
...
async def update_user_group(
    session: AsyncSession, user_id: int, new_group_id: int, recruiter_id: int, bot=None, company_id: int = None
) -> bool:
    ...
    try:
        ...
        if bot:
            await send_notification_about_data_change(
                session, bot, user_id, recruiter_id, "ГРУППА", old_group_name, new_group.name, company_id
            )
...
async def update_user_groups(
    session: AsyncSession, user_id: int, new_group_ids: List[int], recruiter_id: int, bot=None, company_id: int = None
) -> bool:
    ...
    try:
        ...
        query = select(Group).where(Group.id.in_(new_group_ids), Group.is_active == True)
        if company_id is not None:
            query = query.where(Group.company_id == company_id)
        ...
        if set(valid_group_ids) != unique_new_group_ids:
            ...
            logger.error(f"Группы {missing_ids} не найдены или не принадлежат компании {company_id}")
            return False
        ...
        if bot:
            await send_notification_about_data_change(
                session, bot, user_id, recruiter_id, "ГРУППЫ", old_group_names, new_group_names, company_id
            )

        logger.info(f"Группы пользователя {user_id} изменены с '{old_group_names}' на '{new_group_names}'")
        return True
...
async def update_user_internship_object(
    session: AsyncSession, user_id: int, new_object_id: int, recruiter_id: int, bot=None, company_id: int = None
) -> bool:
    ...
    try:
        ...
        if bot:
            await send_notification_about_data_change(
                session, bot, user_id, recruiter_id, "ОБЪЕКТ СТАЖИРОВКИ", old_object_name, new_object.name, company_id
            )
...
async def update_user_work_object(
    session: AsyncSession, user_id: int, new_object_id: int, recruiter_id: int, bot=None, company_id: int = None
) -> bool:
    ...
    try:
        ...
        if bot:
            await send_notification_about_data_change(
                session, bot, user_id, recruiter_id, "ОБЪЕКТ РАБОТЫ", old_object_name, new_object.name, company_id
            )
...
async def send_notification_about_data_change(
    session: AsyncSession,
    bot,
    user_id: int,
    recruiter_id: int,
...
⚠️{new_value}"""
        ...
        if field_name in ["РОЛЬ", "ГРУППА", "ГРУППЫ", "ОБЪЕКТ СТАЖИРОВКИ", "ОБЪЕКТ РАБОТЫ"]:
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[[InlineKeyboardButton(text="🔄 Перезагрузка", callback_data="reload_menu")]]
            )
            await bot.send_message(chat_id=user.tg_id, text=notification_text, parse_mode="HTML", reply_markup=keyboard)
        else:
...
async def create_attestation(
    session: AsyncSession, name: str, passing_score: float, creator_id: int, company_id: int = None
...
async def add_attestation_question(
    session: AsyncSession, attestation_id: int, question_text: str, max_points: float, question_number: int
    ...
    try:
        ...
        try:
            query = (
                ...
                .where(Attestation.is_active == True, Attestation.company_id == company_id)
...
async def save_trajectory_to_database(
    session: AsyncSession, trajectory_data: dict, company_id: int = None
    ...
    try:
        ...
        logger.info(
            f"Сохранение траектории: name={trajectory_data['name']}, group_id={trajectory_data['group_id']}, created_by_id={trajectory_data['created_by_id']}"
        )
        ...
        try:
            ...
            for stage_data in trajectory_data.get("stages", []):
                ...
                stage = LearningStage(
                    name=stage_data["name"], learning_path_id=learning_path.id, order_number=stage_data["order"]
                )
                session.add(stage)
                await session.flush()
                ...
                for session_data in stage_data.get("sessions", []):
                    ...
                    learning_session = LearningSession(
                        name=session_data["name"], stage_id=stage.id, order_number=session_data["order"]
                    )
                    ...
                    for test_data in session_data.get("tests", []):
                        ...
                        stmt = insert(session_tests).values(
                            session_id=learning_session.id, test_id=test_data["id"], order_number=test_data["order"]
                        )
                        await session.execute(stmt)
...
async def save_trajectory_with_attestation_and_group(
    session: AsyncSession, trajectory_data: dict, attestation_id: int, group_id: int, company_id: int = None
) -> bool:
    ...
    try:
        ...
        if attestation_id:
            update_stmt = (
                update(LearningPath).where(LearningPath.id == learning_path.id).values(attestation_id=attestation_id)
            )
            await session.execute(update_stmt)
            await session.commit()
...
async def get_all_learning_paths(session: AsyncSession, company_id: int = None) -> List[LearningPath]:
    ...
    try:
        ...
        if company_id is None:
            logger.warning(
                "get_all_learning_paths вызван с company_id=None - возвращаем пустой список для безопасности"
            )
            return []
        ...
        try:
            query = (
                ...
                .where(LearningPath.is_active == True, LearningPath.company_id == company_id)
...
async def get_learning_path_by_id(
    session: AsyncSession, path_id: int, company_id: int = None
) -> Optional[LearningPath]:
...
async def get_learning_paths_by_group(
    session: AsyncSession, group_id: int, company_id: int = None
) -> List[LearningPath]:
    ...
    try:
        ...
        query = (
            ...
            .where(LearningPath.group_id == group_id, LearningPath.is_active == True)
        )
        ...
        result = await session.execute(query.order_by(LearningPath.created_date.desc()))
        paths = result.scalars().all()
        return list(paths)
...
async def deactivate_learning_path(session: AsyncSession, path_id: int, company_id: int = None) -> bool:
    ...
    try:
        ...
        update_stmt = update(LearningPath).where(LearningPath.id == path_id)
...
async def get_attestation_by_id(
    ...
    try:
        query = select(Attestation).options(selectinload(Attestation.questions)).where(Attestation.id == attestation_id)
...
async def delete_attestation(session: AsyncSession, attestation_id: int, company_id: int = None) -> bool:
    ...
    try:
        ...
        from bot.database.models import Attestation, AttestationQuestion
        ...
        await session.execute(delete(AttestationQuestion).where(AttestationQuestion.attestation_id == attestation_id))

        # Удаляем саму аттестацию
        await session.execute(delete(Attestation).where(Attestation.id == attestation_id))
...
async def get_trajectories_using_attestation(
    session: AsyncSession, attestation_id: int, company_id: int = None
) -> List[str]:
...
async def get_available_mentors_for_trainee(
    session: AsyncSession, trainee_id: int, company_id: int = None
) -> List[User]:
    ...
    try:
        ...
        query_mentors = (
            select(User)
            .options(selectinload(User.work_object), selectinload(User.internship_object))
            .where(User.is_active == True, User.work_object_id == trainee.work_object_id)
            .join(user_roles)
...
async def assign_mentor_to_trainee(
    session: AsyncSession, trainee_id: int, mentor_id: int, recruiter_id: int, bot=None, company_id: int = None
) -> bool:
    """Назначение наставника стажеру"""
    try:
        from bot.database.models import Mentorship, User
        ...
        trainee_query = (
            ...
            .where(User.id == trainee_id, User.is_active == True, User.is_activated == True)
        )
        ...
        mentor_query = (
            ...
            .where(User.id == mentor_id, User.is_active == True)
        )
...
async def send_mentor_assigned_notification(
    session: AsyncSession, trainee_id: int, mentor_id: int, recruiter_id: int, bot=None, company_id: int = None
) -> None:
    ...
    try:
        ...
        # Отправляем уведомления
        try:
            await bot.send_message(trainee.tg_id, trainee_message, parse_mode="HTML")
            await bot.send_message(
                mentor.tg_id,
                mentor_message,
                parse_mode="HTML",
...
async def assign_learning_path_to_trainee(
    session: AsyncSession, trainee_id: int, learning_path_id: int, mentor_id: int, bot=None, company_id: int = None
) -> bool:
    ...
    try:
        ...
        from bot.database.models import TraineeTestAccess, session_tests
        ...
        active_paths_result = await session.execute(
            select(TraineeLearningPath.id).where(
                TraineeLearningPath.trainee_id == trainee_id, TraineeLearningPath.is_active == True
            )
        )
        ...
        if active_path_ids:
            ...
            stage_progress_result = await session.execute(
                select(TraineeStageProgress.id).where(TraineeStageProgress.trainee_path_id.in_(active_path_ids))
            )
            ...
            if stage_progress_ids:
                ...
                await session.execute(
                    delete(TraineeStageProgress).where(TraineeStageProgress.id.in_(stage_progress_ids))
                )
        ...
        if trajectory_test_ids:
            await session.execute(
                delete(TraineeTestAccess).where(
                    TraineeTestAccess.trainee_id == trainee_id, TraineeTestAccess.test_id.in_(trajectory_test_ids)
                )
            )
        ...
        trainee_path = TraineeLearningPath(
            trainee_id=trainee_id, learning_path_id=learning_path_id, assigned_by_id=mentor_id, is_active=True
        )
...
async def _create_trainee_progress(session: AsyncSession, trainee_path_id: int, learning_path_id: int) -> None:
    ...
    try:
        from bot.database.models import LearningSession, LearningStage, TraineeSessionProgress, TraineeStageProgress
...
async def send_learning_path_assigned_notification(
    session: AsyncSession, trainee_id: int, learning_path_id: int, bot=None, company_id: int = None
) -> None:
    """Отправка уведомления стажеру о назначении траектории (с изоляцией по компании)"""
    try:
        from bot.database.models import LearningPath, User
        ...
        if not bot:
            ...
            return
        try:
            await bot.send_message(trainee.tg_id, message, parse_mode="HTML")
...
async def open_stage_for_trainee(
    ...
    try:
        ...
        await session.execute(
            ...
            .where(TraineeStageProgress.trainee_path_id == trainee_path.id, TraineeStageProgress.stage_id == stage_id)
            .values(is_opened=True, opened_date=moscow_now())
        )

        # Открываем все сессии этого этапа
        stage_progress_result = await session.execute(
            select(TraineeStageProgress).where(
                TraineeStageProgress.trainee_path_id == trainee_path.id, TraineeStageProgress.stage_id == stage_id
            )
        )
        ...
        if stage_progress:
            ...
            from bot.database.models import LearningSession
                ...
                for test in session_obj.tests:
                    ...
                    if not existing_access.scalar_one_or_none():
                        ...
                        if final_company_id is not None:
                            ...
                            if not test_obj:
                                logger.warning(
                                    f"Тест {test.id} не найден или не принадлежит компании {final_company_id}, пропускаем"
                                )
                                continue
                        else:
                            logger.warning(
                                f"Не удалось определить company_id для стажера {trainee_id}, пропускаем тест {test.id}"
                            )
                            continue
                        ...
                        tests_granted += 1
                        logger.info(
                            f"ДЕБАГ: Создан доступ к тесту {test.id} для стажера {trainee_id} при открытии этапа {stage_id}"
                        )
                    else:
...
async def send_stage_opened_notification(
    session: AsyncSession, trainee_id: int, stage_id: int, bot=None, company_id: int = None
) -> None:
    """Отправка уведомления стажеру об открытии этапа (с изоляцией по компании)"""
    try:
        from bot.database.models import LearningStage, User
        ...
        message = "🚨<b>Тебе открыли новый этап обучения!</b>\nНажми кнопку ниже, чтобы открыть траекторию"

        # Создаем клавиатуру с кнопкой траектории
        from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup

        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="Траектория обучения ⬜", callback_data="trajectory_command")]]
        )
        ...
        if not bot:
            ...
            return
        try:
            await bot.send_message(trainee.tg_id, message, parse_mode="HTML", reply_markup=keyboard)
...
async def send_stage_completion_notification_to_trainee(
    session: AsyncSession, trainee_id: int, stage_id: int, bot=None, company_id: int = None
) -> None:
    ...
    try:
        ...
        trajectory_progress = await generate_trajectory_progress_with_attestation_status(
...
async def get_trainee_learning_path(
    session: AsyncSession, trainee_id: int, company_id: int = None
) -> Optional[TraineeLearningPath]:
    """Получение активной траектории стажера с загрузкой связанных объектов (с изоляцией по компании)"""
    try:
        from bot.database.models import TraineeLearningPath
        ...
        query = (
            ...
            .options(
                ...
                selectinload(TraineeLearningPath.learning_path).selectinload(
                    LearningPath.attestation
        ...
        if company_id is not None:
            query = query.join(LearningPath, TraineeLearningPath.learning_path_id == LearningPath.id).where(
                LearningPath.company_id == company_id
            )
...
async def get_trainee_stage_progress(
    session: AsyncSession, trainee_path_id: int, company_id: int = None
) -> List[TraineeStageProgress]:
    """Получение прогресса стажера по этапам с загрузкой связанных объектов (с изоляцией по компании)"""
    try:
        from bot.database.models import LearningStage, TraineeLearningPath, TraineeStageProgress
        ...
        query = (
            ...
            .options(
                ...
                selectinload(TraineeStageProgress.session_progress).selectinload(
                    TraineeSessionProgress.session
                ),  # Загружаем сессии
                selectinload(TraineeStageProgress.session_progress)
                .selectinload(TraineeSessionProgress.session)
                .selectinload(LearningSession.tests),  # Загружаем тесты сессий
                selectinload(TraineeStageProgress.trainee_path)
                .selectinload(TraineeLearningPath.learning_path)
                .selectinload(LearningPath.attestation),  # Загружаем аттестацию
            )
            .join(LearningStage)
            .where(TraineeStageProgress.trainee_path_id == trainee_path_id)
        )

        # Изоляция по компании - КРИТИЧЕСКИ ВАЖНО!
        if company_id is not None:
            query = (
                query.join(TraineeLearningPath, TraineeStageProgress.trainee_path_id == TraineeLearningPath.id)
                .join(LearningPath, TraineeLearningPath.learning_path_id == LearningPath.id)
...
async def get_stage_session_progress(session: AsyncSession, stage_progress_id: int) -> List[TraineeSessionProgress]:
    ...
    try:
        from bot.database.models import LearningSession, TraineeSessionProgress

        result = await session.execute(
            select(TraineeSessionProgress)
            .options(
                selectinload(TraineeSessionProgress.session).selectinload(
                    LearningSession.tests
                )  # Загружаем тесты сессии
            )
            .join(LearningSession)
            .where(TraineeSessionProgress.stage_progress_id == stage_progress_id)
            .order_by(LearningSession.order_number)
        )
        return result.scalars().all()
    except Exception as e:
...
async def get_all_stage_sessions_progress(
    session: AsyncSession, stage_progress_id: int
) -> List[TraineeSessionProgress]:
...
async def complete_stage_for_trainee(
    session: AsyncSession, trainee_id: int, stage_id: int, company_id: int = None
) -> bool:
    """Отметка этапа как пройденного (с изоляцией по компании)"""
    try:
        from bot.database.models import LearningPath, TraineeLearningPath, TraineeStageProgress, User
        ...
        await session.execute(
            update(TraineeStageProgress)
            .where(TraineeStageProgress.trainee_path_id == trainee_path.id, TraineeStageProgress.stage_id == stage_id)
            .values(is_completed=True, completed_date=moscow_now())
        )
...
async def complete_session_for_trainee(
    session: AsyncSession, trainee_id: int, session_id: int, company_id: int = None
) -> bool:
        ...
        await session.execute(
            ...
            .where(
                TraineeSessionProgress.stage_progress_id.in_(
                    select(TraineeStageProgress.id).where(TraineeStageProgress.trainee_path_id == trainee_path.id)
                ),
...
async def get_available_learning_paths_for_mentor(
    session: AsyncSession, mentor_id: int, company_id: int = None
) -> List[LearningPath]:
    ...
    try:
        ...
        query = (
            ...
            .where(LearningPath.group_id.in_(mentor_group_ids), LearningPath.is_active == True)
        )
...
async def get_learning_path_stages(
    session: AsyncSession, learning_path_id: int, company_id: int
) -> List[LearningStage]:
    """Получение этапов траектории (строго в рамках компании)"""
    try:
        from bot.database.models import LearningPath, LearningStage
...
async def get_session_with_tests(
    session: AsyncSession, session_id: int, company_id: int
) -> Optional["LearningSession"]:
    """Получение сессии с загруженными тестами (строго в рамках компании)"""
    try:
        from bot.database.models import LearningPath, LearningSession, LearningStage
...
async def get_session_tests(session: AsyncSession, session_id: int, company_id: int = None) -> List[Test]:
    ...
    try:
        from bot.database.models import session_tests
...
async def update_learning_path_name(session: AsyncSession, path_id: int, new_name: str, company_id: int = None) -> bool:
    ...
    try:
        ...
        if not new_name:
            logger.error("Название траектории не может быть пустым")
            return False
...
async def update_learning_stage_name(
    session: AsyncSession, stage_id: int, new_name: str, company_id: int = None
) -> bool:
    ...
    try:
        ...
        new_name = new_name.strip() if new_name else ""
        if not new_name:
            logger.error("Название этапа не может быть пустым")
            return False
        ...
        if company_id is not None:
            query = query.join(LearningPath, LearningStage.learning_path_id == LearningPath.id).where(
                LearningPath.company_id == company_id
            )
...
async def update_learning_stage_description(
    session: AsyncSession, stage_id: int, new_description: Optional[str]
) -> bool:
    """Изменение описания этапа траектории"""
    try:
        # Проверяем существование этапа
        result = await session.execute(select(LearningStage).where(LearningStage.id == stage_id))
        stage = result.scalar_one_or_none()
        if not stage:
            logger.error(f"Этап с ID {stage_id} не найден")
            return False
...
async def update_learning_session_description(
    session: AsyncSession, session_id: int, new_description: Optional[str]
) -> bool:
...
async def update_learning_path_group(
    session: AsyncSession, path_id: int, new_group_id: int, company_id: int = None
) -> bool:
...
async def update_learning_path_attestation(
    session: AsyncSession, path_id: int, new_attestation_id: Optional[int], company_id: int = None
) -> bool:
    ...
    try:
        ...
        for stage_id, new_order in stage_orders.items():
            ...
            stmt = (
                ...
                .where(LearningStage.id == stage_id, LearningStage.learning_path_id == path_id)
...
async def reorder_learning_sessions(
    session: AsyncSession, stage_id: int, session_orders: dict, company_id: int = None
) -> bool:
    ...
    try:
        ...
        for session_id, new_order in session_orders.items():
            ...
            stmt = (
                ...
                .where(LearningSession.id == session_id, LearningSession.stage_id == stage_id)
                .values(order_number=new_order)
            )
            await session.execute(stmt)
...
async def check_stage_has_trainees(session: AsyncSession, stage_id: int, company_id: int = None) -> bool:
    ...
    try:
        ...
        query_count = (
            ...
            .join(User, TraineeLearningPath.trainee_id == User.id)
            .where(
                TraineeLearningPath.learning_path_id == stage.learning_path_id, TraineeLearningPath.is_active == True
            )
        )
        if company_id is not None:
...
async def check_session_has_trainees(session: AsyncSession, session_id: int, company_id: int = None) -> bool:
    ...
    try:
        from bot.database.models import LearningPath, LearningSession, LearningStage, TraineeLearningPath, User
        ...
        query = (
            ...
            .join(User, TraineeLearningPath.trainee_id == User.id)
            .where(LearningSession.id == session_id, TraineeLearningPath.is_active == True)
        )
...
async def add_test_to_session_from_editor(
    ...
    try:
        ...
        stage_result = await session.execute(select(LearningStage).where(LearningStage.id == learning_session.stage_id))
        stage = stage_result.scalar_one_or_none()
        if not stage:
        ...
        existing_result = await session.execute(
            select(session_tests).where(session_tests.c.session_id == session_id, session_tests.c.test_id == test_id)
        )
        ...
        max_order_result = await session.execute(
            select(func.max(session_tests.c.order_number)).where(session_tests.c.session_id == session_id)
        )
        max_order = max_order_result.scalar() or 0

        # Добавляем тест в сессию
        stmt = insert(session_tests).values(session_id=session_id, test_id=test_id, order_number=max_order + 1)
        await session.execute(stmt)
        await session.commit()
...
async def remove_test_from_session(
    session: AsyncSession, session_id: int, test_id: int, company_id: int = None
) -> bool:
    ...
    try:
        ...
        await session.execute(
            delete(session_tests).where(session_tests.c.session_id == session_id, session_tests.c.test_id == test_id)
        )
        await session.commit()
...
async def save_stage_to_trajectory_from_editor(
    session: AsyncSession, path_id: int, stage_data: dict
    ...
    try:
        ...
        for session_data in stage_data.get("sessions", []):
            ...
            for test_data in session_data.get("tests", []):
                ...
                stmt = insert(session_tests).values(
                    session_id=learning_session.id, test_id=test_id, order_number=test_order
                )
                await session.execute(stmt)
...
async def get_available_managers_for_trainee(
    session: AsyncSession, trainee_id: int, company_id: int = None
) -> List[User]:
    """
    ...
    try:
        ...
        manager_role_result = await session.execute(select(Role).where(Role.name == "Руководитель"))
...
async def assign_manager_to_trainee(
    session: AsyncSession, trainee_id: int, manager_id: int, assigned_by_id: int, company_id: int = None
) -> Optional[TraineeManager]:
    """
    ...
    try:
        ...
        trainee_manager = TraineeManager(trainee_id=trainee_id, manager_id=manager_id, assigned_by_id=assigned_by_id)
...
async def get_trainee_manager(
    ...
    try:
        ...
        query = select(TraineeManager).where(TraineeManager.trainee_id == trainee_id, TraineeManager.is_active == True)

        # Изоляция по компании - КРИТИЧЕСКИ ВАЖНО!
        # Проверяем через join с User (manager)
        if company_id is not None:
            query = query.join(User, TraineeManager.manager_id == User.id).where(User.company_id == company_id)
...
async def get_manager_trainees(session: AsyncSession, manager_id: int, company_id: int = None) -> List[TraineeManager]:
    ...
    try:
        query = (
            ...
            .where(TraineeManager.manager_id == manager_id, TraineeManager.is_active == True)
        )
...
async def conduct_attestation(
    session: AsyncSession, trainee_id: int, attestation_id: int, manager_id: int, scores: dict, company_id: int = None
...
async def get_user_attestation_result(
    ...
    try:
        ...
        result = await session.execute(query.order_by(AttestationResult.completed_date.desc()))
        return result.scalar_one_or_none()
    except Exception as e:
...
async def change_trainee_to_employee(
    session: AsyncSession, trainee_id: int, attestation_result_id: int, company_id: int = None
) -> bool:
    """
    ...
    """
    try:
        from bot.database.models import LearningPath, Mentorship, TraineeLearningPath
        ...
        employee_role_result = await session.execute(select(Role).where(Role.name == "Сотрудник"))
        ...
        await session.execute(
            user_roles.delete().where(user_roles.c.user_id == trainee_id, user_roles.c.role_id == trainee_role.id)
        )

        # Добавляем роль сотрудника
        stmt = insert(user_roles).values(user_id=trainee_id, role_id=employee_role.id)
        await session.execute(stmt)
        ...
        mentorship_update_query = update(Mentorship).where(
            Mentorship.trainee_id == trainee_id, Mentorship.is_active == True
        )
        ...
        deleted_results = await session.execute(delete(TestResult).where(TestResult.user_id == trainee_id))
        if deleted_results.rowcount > 0:
            logger.info(f"Очищено {deleted_results.rowcount} результатов тестов при переходе в сотрудники")

        await session.commit()
        logger.info(
            f"Роль стажера изменена на сотрудника для пользователя {trainee.full_name}. Траектории, наставничество и результаты тестов деактивированы."
        )
        return True
...
async def make_manager_decision(
    session: AsyncSession, attestation_result_id: int, decision: bool, comment: str = None, company_id: int = None
) -> bool:
    """
    Принятие решения руководителем о переводе стажера в сотрудники (с изоляцией по компании)
    """
    try:
        from bot.database.models import Attestation, AttestationResult, User
        ...
        if not attestation_result:
            logger.error(
                f"Результат аттестации {attestation_result_id} не найден или не принадлежит компании {company_id}"
            )
            return False
        ...
        logger.info(
            f"Руководитель принял решение по аттестации {attestation_result_id}: {'перевести в сотрудники' if decision else 'оставить стажером'}"
        )
        return True
...
async def get_pending_attestation_decisions(
...
async def send_attestation_completed_notification(
    session: AsyncSession, trainee_id: int, attestation_result: AttestationResult, company_id: int = None
) -> None:
    """
...
async def broadcast_test_to_groups(
    session: AsyncSession,
    test_id: int,
    group_ids: list,
    sent_by_id: int,
    bot=None,
    broadcast_script: str = None,
    broadcast_photos: list = None,
    broadcast_material_id: int = None,
    broadcast_docs: list | None = None,
        ...
        if test_id:
            try:
                ...
                if user_ids:  # Защита от пустого списка
                    existing_result = await session.execute(
                        select(TraineeTestAccess.trainee_id, TraineeTestAccess.test_id).where(
                            TraineeTestAccess.test_id == test_id,
                    ...
                    if new_accesses:
                        await session.execute(insert(TraineeTestAccess).values(new_accesses))
                        await session.commit()
                        logger.info(
                            f"Массово создано {len(new_accesses)} новых доступов к тесту {test_id} для пользователей: {[acc['trainee_id'] for acc in new_accesses[:5]]}{'...' if len(new_accesses) > 5 else ''}"
                        )
                    else:
                        logger.info(
                            f"Все доступы к тесту {test_id} уже существуют для выбранных пользователей ({len(final_users)} пользователей)"
                        )
                else:
                    logger.warning("Список пользователей для рассылки пуст")
...
async def create_knowledge_folder(
    session: AsyncSession, name: str, created_by_id: int, description: str = None, company_id: int = None
    ...
    try:
        ...
        existing_query = select(KnowledgeFolder).where(
            func.lower(KnowledgeFolder.name) == func.lower(name), KnowledgeFolder.is_active == True
        )
        ...
        folder = KnowledgeFolder(name=name, description=description, created_by_id=created_by_id, company_id=company_id)
        session.add(folder)
        await session.flush()
...
async def get_all_knowledge_folders(session: AsyncSession, company_id: int = None) -> List[KnowledgeFolder]:
    ...
    try:
        query = (
            ...
            .options(selectinload(KnowledgeFolder.materials), selectinload(KnowledgeFolder.accessible_groups))
        )
...
async def get_knowledge_folder_by_id(
    session: AsyncSession, folder_id: int, company_id: int = None
) -> Optional[KnowledgeFolder]:
...
async def update_knowledge_folder_name(
    session: AsyncSession, folder_id: int, new_name: str, updated_by_id: int, company_id: int = None
) -> bool:
...
async def create_knowledge_material(
    ...
    try:
        ...
        result = await session.execute(
            select(func.max(KnowledgeMaterial.order_number)).where(
                KnowledgeMaterial.folder_id == folder_id, KnowledgeMaterial.is_active == True
            )
        )
...
async def set_folder_access_groups(
    session: AsyncSession, folder_id: int, group_ids: List[int], updated_by_id: int, company_id: int = None
) -> bool:
    ...
    try:
        ...
        await session.execute(delete(folder_group_access).where(folder_group_access.c.folder_id == folder_id))
        ...
        if group_ids:
            ...
            for group_id in existing_group_ids:
                await session.execute(insert(folder_group_access).values(folder_id=folder_id, group_id=group_id))
...
async def check_folder_access(session: AsyncSession, folder_id: int, user_id: int, company_id: int = None) -> bool:
    ...
    try:
        ...
        query = select(User).where(User.id == user_id, User.is_active == True).options(selectinload(User.groups))
...
async def get_accessible_knowledge_folders_for_user(
    session: AsyncSession, user_id: int, company_id: int = None
    ...
    try:
        ...
        user_query = select(User).where(User.id == user_id, User.is_active == True).options(selectinload(User.groups))
        ...
        logger.info(
            f"Пользователь {user.full_name} имеет доступ к {len(accessible_folders)} папкам из {len(all_folders)}"
        )
...
async def get_folder_access_info(session: AsyncSession, folder_id: int, company_id: int = None) -> dict:
    ...
    try:
        ...
        if not folder.accessible_groups:
            return {"success": True, "access_type": "all_groups", "description": "все группы", "groups": []}
        else:
...
async def fix_knowledge_base_permissions(session: AsyncSession) -> bool:
    ...
    try:
        ...
        result = await session.execute(select(Permission).where(Permission.name == "view_knowledge_base"))
        permission = result.scalar_one_or_none()

        if not permission:
            # Создаем право если его нет
            permission = Permission(name="view_knowledge_base", description="Просмотр базы знаний")
            session.add(permission)
            await session.flush()
        ...
        for role_name in roles_to_update:
            ...
            if role:
                ...
                if not existing_link:
                    # Добавляем связь
                    stmt = insert(role_permissions).values(role_id=role.id, permission_id=permission.id)
                    await session.execute(stmt)
...
async def fix_recruiter_take_tests_permission(session: AsyncSession, company_id: int = None) -> bool:
    ...
    try:
        ...
        result = await session.execute(select(Permission).where(Permission.name == "take_tests"))
        permission = result.scalar_one_or_none()
        ...
        result = await session.execute(select(Role).where(Role.name == "Рекрутер"))
        role = result.scalar_one_or_none()
...
async def get_trajectory_usage_info(session: AsyncSession, trajectory_id: int, company_id: int = None) -> dict:
    ...
    try:
        from bot.database.models import LearningPath, TraineeLearningPath, User
        ...
        return {"trainees": trainees, "trainees_count": len(trainees), "total_users": total_users}
...
async def get_company_by_invite_code(session: AsyncSession, invite_code: str) -> Optional[Company]:
    ...
    try:
        ...
        result = await session.execute(select(Company).where(func.upper(Company.invite_code) == invite_code_normalized))
        company = result.scalar_one_or_none()

        if not company:
            logger.warning(
                f"Компания с кодом приглашения '{invite_code}' (нормализовано: '{invite_code_normalized}') не найдена"
            )
...
async def check_company_access(session: AsyncSession, company_id: int) -> dict:
    ...
    try:
        ...
        if not company:
            return {"accessible": False, "reason": "company_not_found", "company": None}

        if not company.is_active:
            return {"accessible": False, "reason": "company_inactive", "company": company}

        if not company.subscribe:
            return {"accessible": False, "reason": "subscription_expired", "company": company}
        ...
        if company.members >= company.members_limit:
            return {"accessible": False, "reason": "members_limit_reached", "company": company}

        return {"accessible": True, "reason": None, "company": company}

    except Exception as e:
        logger.error(f"Ошибка проверки доступа к компании {company_id}: {e}")
        return {"accessible": False, "reason": "error", "company": None}
...
async def update_company_members_count(session: AsyncSession, company_id: int):
    ...
    try:
        result = await session.execute(
            select(func.count(User.id)).where(and_(User.company_id == company_id, User.is_active == True))
        )
        count = result.scalar()

        await session.execute(update(Company).where(Company.id == company_id).values(members=count))
    ...
    try:
        result = await session.execute(
            select(Company).where(
                and_(Company.subscribe == True, Company.finish_date <= moscow_now(), Company.is_active == True)
            )
        )
        return result.scalars().all()
    except Exception as e:
...
async def create_user_with_company(
    session: AsyncSession, user_data: dict, company_id: int, role: Optional[str] = None, bot=None
) -> Optional[User]:
    ...
    try:
        ...
        if role:
            ...
            if role_obj:
                ...
                stmt = insert(user_roles).values(user_id=user.id, role_id=role_obj.id)
                await session.execute(stmt)
            else:
        ...
        # Используем явное обновление через update() для надежности в асинхронном контексте
        await session.execute(update(Company).where(Company.id == company_id).values(members=new_members_count))
...
async def get_company_recruiters(session: AsyncSession, company_id: int) -> List[User]:
    ...
    try:
        result = await session.execute(
            select(User)
            .join(User.roles)
            .where(and_(User.company_id == company_id, User.is_active == True, Role.name == "Рекрутер"))
        )
        return result.scalars().all()
    except Exception as e:
...
async def update_company_name(
    session: AsyncSession, company_id: int, new_name: str, company_id_check: int = None
) -> bool:
    ...
    try:
        ...
        # Обновление
        await session.execute(update(Company).where(Company.id == company_id).values(name=new_name))
        await session.commit()
...
async def update_company_description(
    session: AsyncSession, company_id: int, new_description: str, company_id_check: int = None
) -> bool:
    ...
    try:
        ...
        # Обновление
        await session.execute(update(Company).where(Company.id == company_id).values(description=new_description))
        await session.commit()
...
async def update_company_invite_code(
    session: AsyncSession, company_id: int, new_invite_code: str, company_id_check: int = None
) -> bool:
    ...
    try:
        ...
        # Обновление
        await session.execute(update(Company).where(Company.id == company_id).values(invite_code=new_invite_code))
        await session.commit()
    ...
    try:
        ...
        if existing_company:
            ...
            if max_id is not None:
                await session.execute(
                    text("SELECT setval('companies_id_seq', :max_id, true)").bindparams(max_id=max_id)
                )
                await session.flush()
...
async def migrate_existing_users_to_default_company(session: AsyncSession) -> int:
    ...
    try:
        ...
        result = await session.execute(select(User).where(User.company_id.is_(None)))
        ...
        if migrated_count > 0:
            await session.execute(update(User).where(User.id.in_(user_ids)).values(company_id=1))
