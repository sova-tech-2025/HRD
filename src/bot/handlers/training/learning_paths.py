...
from bot.database.db import (
    ...
    get_trajectory_usage_info,
    get_user_by_tg_id,
)
from bot.keyboards.keyboards import (
    get_attestation_creation_start_keyboard,
    get_attestation_questions_keyboard,
    get_attestation_selection_keyboard,
    get_attestations_main_keyboard,
    get_group_selection_keyboard,
    get_learning_paths_main_keyboard,
    get_main_menu_keyboard,
    get_more_questions_keyboard,
    get_question_type_keyboard,
    get_session_management_keyboard,
    get_test_creation_cancel_keyboard,
    get_test_description_skip_keyboard,
    get_test_materials_choice_keyboard,
    get_test_materials_skip_keyboard,
    get_test_selection_keyboard,
    get_trajectory_attestation_confirmation_keyboard,
    get_trajectory_creation_start_keyboard,
    get_trajectory_deletion_confirmation_keyboard,
    get_trajectory_final_confirmation_keyboard,
    get_trajectory_save_confirmation_keyboard,
    get_trajectory_selection_keyboard,
    is_main_menu_text,
)
from bot.repositories import AssessmentRepository
from bot.states.states import AttestationStates, LearningPathStates
...
async def callback_create_trajectory(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            instruction_text, reply_markup=get_trajectory_creation_start_keyboard(), parse_mode="HTML"
        )

        # Инициализируем данные траектории в state
        await state.update_data(
            trajectory_data={"name": "", "stages": [], "created_by_id": None},
...
async def process_stage_name(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        # Создаем новый этап
        new_stage = {"name": stage_name, "order": current_stage_number, "sessions": []}
...
async def process_session_name(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        new_session = {"name": session_name, "order": current_session_number, "tests": []}
        ...
        await message.answer(text, reply_markup=get_test_selection_keyboard(tests, []), parse_mode="HTML")
...
async def callback_create_new_test(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(text, reply_markup=get_test_creation_cancel_keyboard(), parse_mode="HTML")
...
async def process_new_test_name(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await message.answer(text, reply_markup=get_test_materials_choice_keyboard(), parse_mode="HTML")
...
async def callback_add_materials(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(text, reply_markup=get_test_materials_skip_keyboard(), parse_mode="HTML")
...
async def show_test_description_step(message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(text, reply_markup=get_test_description_skip_keyboard(), parse_mode="HTML")
...
async def show_question_type_step(message, state: FSMContext):
    ...
    try:
        ...
        # Всегда отправляем новое сообщение, так как это может быть message от пользователя
        await message.answer(text, reply_markup=get_question_type_keyboard(), parse_mode="HTML")
                    ...
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="✅ Завершить добавление вариантов", callback_data="finish_trajectory_options"
                            )
                        ]
                    ]
                ),
...
async def process_question_points(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await state.update_data(
            new_test_question_points=points, new_test_questions=existing_questions, new_test_total_score=total_score
        )
        ...
        await message.answer(text, reply_markup=get_more_questions_keyboard(), parse_mode="HTML")
...
async def callback_add_more_questions(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        log_user_action(
            ...
            "trajectory_test_add_more_questions",
            "Добавление еще одного вопроса к тесту в траектории",
        )
    ...
    try:
        ...
        if company_id is None:
            ...
            log_user_error(
                ...
                "test_creation_company_id_error",
                f"Не удалось получить company_id для пользователя {message.from_user.id} при создании теста",
            )
            return
        ...
        if trajectory_data.get("stages"):
            ...
            if last_stage.get("sessions"):
                ...
                last_session["tests"].append(
                    {"id": test.id, "name": test.name, "order": len(last_session["tests"]) + 1}
                )
        ...
        await message.answer(
            text, reply_markup=get_test_selection_keyboard(tests, current_session_tests), parse_mode="HTML"
        )
    ...
    try:
        ...
        if trajectory_data.get("stages"):
            ...
            if last_stage.get("sessions"):
                last_session = last_stage["sessions"][-1]
                last_session["tests"].append(
                    {"id": test_id, "name": selected_test.name, "order": len(last_session["tests"]) + 1}
                )
...
async def callback_save_session(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(text, reply_markup=get_session_management_keyboard(), parse_mode="HTML")
...
async def callback_save_trajectory(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_trajectory_save_confirmation_keyboard(), parse_mode="HTML"
        )
...
async def callback_confirm_trajectory_save(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_attestation_selection_keyboard(attestations), parse_mode="HTML"
        )
...
async def callback_select_attestation(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        try:
            await callback.message.edit_text(
                text, reply_markup=get_trajectory_attestation_confirmation_keyboard(), parse_mode="HTML"
            )
...
@router.callback_query(
    F.data == "confirm_attestation_and_proceed", LearningPathStates.waiting_for_attestation_confirmation
)
async def callback_confirm_attestation_and_proceed(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_group_selection_keyboard(groups, page=0), parse_mode="HTML"
        )
...
async def callback_select_group_for_trajectory(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await state.update_data(
            selected_group_id=group_id, selected_group_name=group_name, selected_attestation_name=attestation_name
        )
        ...
        await callback.message.edit_text(
            text, reply_markup=get_trajectory_final_confirmation_keyboard(), parse_mode="HTML"
        )
...
async def callback_final_confirm_save(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        success = await AssessmentRepository(session).save_with_trajectory_and_group(
            trajectory_data, attestation_id, group_id, company_id
        )
        ...
        log_user_action(
            callback.from_user.id, "trajectory_created_final", f"Финально создана траектория: {trajectory_data['name']}"
        )
...
@router.callback_query(
    F.data == "cancel_attestation_confirmation", LearningPathStates.waiting_for_attestation_confirmation
)
...
async def callback_edit_trajectory(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        from bot.keyboards.keyboards import get_trajectory_selection_for_editor_keyboard
...
@router.callback_query(
    F.data.startswith("trajectories_page_prev:") | F.data.startswith("trajectories_page_next:"),
...
async def callback_edit_specific_trajectory(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        from bot.handlers.training.trajectory_editor import _show_editor_main_menu
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "edit_specific_trajectory_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_create_attestation(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_attestation_creation_start_keyboard(), parse_mode="HTML"
        )
...
async def process_attestation_question(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if len(questions) > 3:
            questions_text = (
                f"📝 <i>Добавлено вопросов: {len(questions) - 3} + показаны последние 3:</i>\n\n" + questions_text
            )
        ...
        await message.answer(text, reply_markup=get_attestation_questions_keyboard(), parse_mode="HTML")
...
async def process_attestation_passing_score(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        attestation = await assessment_repo.create(
        ...
        company_id = await ensure_company_id(session, state, message.from_user.id)
        updated_attestations = await AssessmentRepository(session).get_all(company_id)
        ...
        await message.answer(
            menu_text, reply_markup=get_attestations_main_keyboard(updated_attestations), parse_mode="HTML"
        )
...
async def render_attestation_page(
    ...
    using_trajectories = await AssessmentRepository(session).get_trajectories_using(
        attestation_id, company_id=company_id
    )
    ...
    if total_questions > 3:
        ...
        if page > 0:
            nav_row.append(
                InlineKeyboardButton(text="⬅️ Назад", callback_data=f"attestation_page_prev:{attestation_id}")
            )
        if page < total_pages - 1:
            nav_row.append(
                InlineKeyboardButton(text="Вперёд ➡️", callback_data=f"attestation_page_next:{attestation_id}")
            )
        if nav_row:
...
async def callback_delete_attestation_confirm(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        is_in_use = await AssessmentRepository(session).check_in_use(attestation_id, company_id=company_id)

        if is_in_use:
            # Получаем названия траекторий, использующих эту аттестацию
            trajectory_names = await AssessmentRepository(session).get_trajectories_using(
                attestation_id, company_id=company_id
            )
            ...
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="↩️ Назад к аттестации", callback_data=f"view_attestation:{attestation_id}"
                        )
                    ],
            ...
            questions_count = (
                len(attestation.questions) if hasattr(attestation, "questions") and attestation.questions else 0
            )
            ...
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="🗑️ Да, удалить", callback_data=f"confirm_delete_attestation:{attestation_id}"
                        )
                    ],
        ...
        log_user_action(
            callback.from_user.id, "delete_attestation_requested", f"ID: {attestation_id}, В использовании: {is_in_use}"
        )
...
@router.callback_query(
    F.data.startswith("confirm_delete_attestation:"), AttestationStates.waiting_for_delete_confirmation
)
async def callback_confirm_delete_attestation(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        # Выполняем удаление
        success = await AssessmentRepository(session).delete(attestation_id, company_id=company_id)
        ...
        if success:
            ...
            await callback.message.answer(
                menu_text, reply_markup=get_attestations_main_keyboard(attestations), parse_mode="HTML"
            )

            await state.set_state(AttestationStates.main_menu)

            log_user_action(
                callback.from_user.id, "attestation_deleted", f"'{attestation_name}' (ID: {attestation_id})"
            )
            ...
            log_user_error(
                callback.from_user.id, "attestation_deletion_failed", f"'{attestation_name}' (ID: {attestation_id})"
            )
...
async def callback_back_to_trajectories_main_from_attestations(
    ...
    try:
        ...
        if not trajectories:
            await callback.message.edit_text(
                ...
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_trajectories_main")]]
                ),
            )
            return
        ...
        await callback.message.edit_text(
            text, parse_mode="HTML", reply_markup=get_trajectory_selection_keyboard(trajectories)
        )
...
async def callback_select_trajectory_to_delete(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        warning_text = "⚠️ <b>ПРЕДУПРЕЖДЕНИЕ</b> ⚠️\n\n"
        warning_text += f"<b>Траектория:</b> {trajectory.name}\n\n"

        if usage_info["total_users"] > 0:
            warning_text += "⚠️ <b>ВНИМАНИЕ! Эта траектория активно используется:</b>\n\n"
        ...
        await callback.message.edit_text(
            warning_text, parse_mode="HTML", reply_markup=get_trajectory_deletion_confirmation_keyboard(trajectory_id)
        )
...
async def callback_confirm_trajectory_deletion(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if success:
            await callback.message.edit_text(
                f"✅ <b>Траектория удалена</b>\n\nТраектория '{trajectory.name}' успешно удалена из системы.",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
            ...
            log_user_action(
                callback.from_user.id, "deleted_trajectory", f"trajectory_id: {trajectory_id}, name: {trajectory.name}"
            )
        else:
            await callback.message.edit_text(
...
async def callback_back_to_trajectories_main_universal(
    callback: CallbackQuery, state: FSMContext, session: AsyncSession
):
