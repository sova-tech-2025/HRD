...
from bot.keyboards.keyboards import (
    get_attestation_selection_for_trajectory_keyboard,
    get_back_to_editor_keyboard,
    get_group_selection_for_trajectory_keyboard,
    get_session_deletion_confirmation_keyboard,
    get_session_tests_keyboard,
    get_stage_deletion_confirmation_keyboard,
    get_stage_editor_keyboard,
    get_test_selection_for_session_keyboard,
...
from bot.utils.formatters.trajectory import (
    ...
    format_stage_editor_view,
    format_trajectory_structure,
...
async def render_attestation_page_for_editor(
        ...
        if page > 0:
            nav_row.append(
                InlineKeyboardButton(
                    text="⬅️ Назад", callback_data=f"editor_attestation_page_prev:{path_id}:{attestation_id}:{page - 1}"
                )
            )
        if page < total_pages - 1:
            nav_row.append(
                InlineKeyboardButton(
                    text="Вперёд ➡️", callback_data=f"editor_attestation_page_next:{path_id}:{attestation_id}:{page + 1}"
                )
            )
        if nav_row:
...
async def _show_editor_main_menu(
    message: Message, state: FSMContext, session: AsyncSession, path_id: int, user_id: int
):
...
async def _show_session_editor(
    message: Message, state: FSMContext, session: AsyncSession, session_id: int, user_id: int
):
    ...
    result = await session.execute(
        ...
        .options(selectinload(LearningSession.stage).selectinload(LearningStage.learning_path))
    )
...
async def callback_back_to_trajectory_list_from_editor(
    ...
    try:
        ...
        from bot.handlers.training.learning_paths import callback_edit_trajectory
    ...
    try:
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[[InlineKeyboardButton(text="⬅️ Отмена", callback_data=f"edit_stage_view:{stage_id}")]]
            ),
        )
...
async def callback_confirm_delete_stage(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            "✅ <b>Этап успешно удален</b>", parse_mode="HTML", reply_markup=get_back_to_editor_keyboard(path_id)
        )
...
async def callback_edit_session_name(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "edit_session_name_start_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_delete_session(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        result = await session.execute(
            select(LearningSession).where(LearningSession.id == session_id).options(selectinload(LearningSession.stage))
        )
        learning_session = result.scalar_one_or_none()
...
async def callback_confirm_delete_session(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "confirm_delete_session_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_add_test_to_session(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        text = "📝 <b>Добавление теста в сессию</b>\n\nВыбери тест для добавления:"
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "add_test_to_session_start_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_select_test_for_session(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "select_test_for_session_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_remove_test_from_session(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        from bot.database.db import get_test_by_id
        ...
        log_user_action(
            callback.from_user.id, "removed_test_from_session", f"Session ID: {session_id}, Test ID: {test_id}"
        )

    except (ValueError, IndexError) as e:
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "remove_test_from_session_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_edit_trajectory_attestation(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        text = f"🔍 <b>Управление аттестацией траектории</b>\n\n<b>Траектория:</b> {learning_path.name}\n\n"
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "edit_trajectory_attestation_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
@router.callback_query(
    F.data.startswith("add_trajectory_attestation:") | F.data.startswith("replace_trajectory_attestation:")
)
async def callback_select_attestation_for_trajectory(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "select_attestation_for_trajectory_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_confirm_attestation_selection(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        log_user_action(
            ...
            "updated_trajectory_attestation",
            f"Path ID: {path_id}, Attestation ID: {attestation_id}",
        )

    except (ValueError, IndexError) as e:
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "confirm_attestation_selection_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_view_trajectory_attestation(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        text, keyboard = await render_attestation_page_for_editor(
            session, attestation_id, path_id, page=0, company_id=user.company_id
        )
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "view_trajectory_attestation_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_editor_attestation_page_prev(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        text, keyboard = await render_attestation_page_for_editor(
            session, attestation_id, path_id, new_page, company_id=user.company_id
        )
        await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="HTML")
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "editor_attestation_page_prev_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_editor_attestation_page_next(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "editor_attestation_page_next_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_remove_trajectory_attestation(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "remove_trajectory_attestation_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_edit_trajectory_group(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        text = "🗂️ <b>Выбор группы для траектории</b>\n\nВыбери группу:"
...
@router.callback_query(
    F.data.startswith("select_group_for_trajectory:"), LearningPathStates.selecting_group_for_trajectory
)
async def callback_confirm_group_selection(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "confirm_group_selection_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def callback_add_stage_to_trajectory(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[[InlineKeyboardButton(text="⬅️ Отмена", callback_data=f"editor_main_menu:{path_id}")]]
            ),
        )
        ...
        await callback.answer("Ошибка при обработке данных", show_alert=True)
        log_user_error(
            callback.from_user.id, "add_stage_to_trajectory_error", f"Invalid data: {callback.data}, {str(e)}"
        )
    except Exception as e:
        await callback.answer("Произошла ошибка")
...
async def process_creating_stage_name(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        result = await session.execute(select(LearningPath).where(LearningPath.id == path_id))
        ...
        new_stage = LearningStage(name=stage_name, description="", learning_path_id=path_id, order_number=stage_order)
        session.add(new_stage)
        await session.commit()

        await message.answer(
            f"✅ Этап '{stage_name}' создан\n\nТеперь добавь сессии к этому этапу.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
...
async def callback_add_session_to_stage(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        result = await session.execute(
            select(LearningStage)
            .where(LearningStage.id == stage_id)
            .options(selectinload(LearningStage.learning_path), selectinload(LearningStage.sessions))
        )
        stage = result.scalar_one_or_none()
...
async def process_creating_session_name(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        new_session = LearningSession(name=session_name, description="", stage_id=stage_id, order_number=session_order)
        session.add(new_session)
        await session.commit()

        await message.answer(
            f"✅ Сессия '{session_name}' создана\n\nТеперь добавь тесты к этой сессии.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
        ...
        log_user_action(
            message.from_user.id, "created_session_in_editor", f"Stage ID: {stage_id}, Session: {session_name}"
        )
