...
async def show_role_selection(callback: CallbackQuery, state: FSMContext, session: AsyncSession, user_id: int):
    ...
    for role in available_roles:
        keyboard_buttons.append([InlineKeyboardButton(text=role.name, callback_data=f"select_role:{role.name}")])

    # Добавляем кнопку "Назад"
    keyboard_buttons.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_user_selection")])
...
async def show_group_selection(
    ...
    keyboard_buttons = []
    for group in groups:
        keyboard_buttons.append([InlineKeyboardButton(text=group.name, callback_data=f"select_group:{group.id}")])

    # Добавляем кнопку "Назад"
    keyboard_buttons.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_role_selection")])
...
async def show_work_object_selection(
    callback: CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    user_id: int,
    ...
    keyboard_buttons = []
    for obj in objects:
        keyboard_buttons.append([InlineKeyboardButton(text=obj.name, callback_data=f"select_work_object:{obj.id}")])

    # Добавляем кнопку "Назад"
    keyboard_buttons.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_previous_step")])
...
async def callback_start_search_new_users(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            "🔍 <b>Поиск новых пользователей</b>\n\nВведи ФИО для поиска (минимум 2 символа):", parse_mode="HTML"
        )
...
async def process_search_query_new_users(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if len(query) < 2:
            await message.answer(
                "❌ Запрос слишком короткий\n\nПожалуйста, введи минимум 2 символа для поиска:", parse_mode="HTML"
            )
            return
...
async def process_group_selection(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    from bot.database.db import get_group_by_id
    ...
    if role_name == "Стажер":
        ...
        for obj in objects:
            keyboard_buttons.append(
                [InlineKeyboardButton(text=obj.name, callback_data=f"select_internship_object:{obj.id}")]
            )

        # Добавляем кнопку "Назад"
        keyboard_buttons.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_group_selection")])

        keyboard = InlineKeyboardMarkup(inline_keyboard=keyboard_buttons)

        registration_date = (
            user.registration_date.strftime("%d.%m.%Y %H:%M") if user.registration_date else "Неизвестно"
        )
...
@router.callback_query(
    UserActivationStates.waiting_for_internship_object_selection, F.data == "back_to_group_selection"
)
...
@router.callback_query(
    UserActivationStates.waiting_for_internship_object_selection, F.data.startswith("select_internship_object:")
)
async def process_internship_object_selection(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    from bot.database.db import get_group_by_id, get_object_by_id
...
async def process_activation_confirmation(callback: CallbackQuery, state: FSMContext, session: AsyncSession, bot):
    ...
    success = await activate_user(
        session, user_id, role_name, group_id, internship_object_id, work_object_id, company_id=company_id, bot=bot
    )
    ...
    if success:
        ...
        await callback.message.edit_text(success_text, reply_markup=get_main_menu_keyboard())
        ...
        await callback.message.edit_text(
            "❌ Произошла ошибка при активации пользователя. Попробуй еще раз.", reply_markup=get_main_menu_keyboard()
        )

        log_user_error(
            callback.from_user.id, callback.from_user.username, "user activation failed", {"user_id": user_id}
        )
...
async def process_activation_cancellation(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    log_user_action(
        callback.from_user.id, callback.from_user.username, "cancelled user activation", {"user_id": user_id}
    )
...
async def callback_show_new_users(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    if not user:
        await callback.answer(
            "Ты не зарегистрирован в системе. Используй команду /register для регистрации.", show_alert=True
        )
        return
