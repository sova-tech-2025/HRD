...
from bot.database.db import (
    ...
    update_user_group,
    update_user_groups,
    update_user_internship_object,
    update_user_phone_number,
    update_user_role,
    update_user_work_object,
)
...
async def show_user_info_detail(
    callback: CallbackQuery, user_id: int, session: AsyncSession, filter_type: str = "all", company_id: int = None
):
    ...
    if role_name in ["Стажер", "Стажёр"]:
        if user.internship_object:
            ...
            text += "<b>Стажировки:</b> Не указан\n"

    if user.work_object:
        text += f"<b>Работы:</b> {user.work_object.name}\n"
    else:
        text += "<b>Работы:</b> Не указан\n"
...
async def cmd_all_users(message: Message, session: AsyncSession, state: FSMContext):
    ...
    log_user_action(
        ...
        "opened_user_filters",
        f"Available: {len(users)} users, {len(groups)} groups, {len(objects)} objects",
    )
    ...
    try:
        ...
        if success:
            ...
            log_user_action(
                ...
                "view_user_info",
                f"User: {user.full_name if user else 'Unknown'} (ID: {user_id})",
            )
...
async def show_user_editor(message: Message, session: AsyncSession, target_user, state: FSMContext):
...
🧑 ФИО: {target_user.full_name}"""
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="cancel_edit")]]
    )
...
Отметь галочками нужные группы и нажми "Сохранить"."""
        ...
        await state.update_data(edit_type="groups", old_value=current_groups, selected_group_ids=selected_group_ids)
    else:
...
async def process_new_group(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    if callback.data.startswith("user_edit_toggle_group:"):
        ...
        new_group_names = ", ".join(groups_dict.get(gid, f"ID:{gid}") for gid in selected_group_ids)
...
async def process_confirm_change(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    if edit_type == "full_name":
        success = await update_user_full_name(
            session, editing_user_id, new_value, recruiter.id, bot, company_id=company_id
        )
        ...
        if existing_user and existing_user.id != editing_user_id:
            ...
            success = await update_user_phone_number(
                session, editing_user_id, new_value, recruiter.id, bot, company_id=company_id
            )
...
"""
            ...
            if role_name in ["Стажер", "Стажёр"]:
                if target_user.internship_object:
                    ...
                    success_message += "<b>Стажировки:</b> Не указан\n"

            # Объект работы
            if target_user.work_object:
                success_message += f"<b>Работы:</b> {target_user.work_object.name}\n"
            else:
                success_message += "<b>Работы:</b> Не указан\n"
            ...
            log_user_action(
                callback.from_user.id, f"edit_user_{edit_type}", f"Changed {edit_type} for user {editing_user_id}"
            )
    else:
        ...
        log_user_error(
            callback.from_user.id,
            f"edit_user_{edit_type}_failed",
...
async def callback_delete_user(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            warning_text, reply_markup=get_user_deletion_confirmation_keyboard(user_id), parse_mode="HTML"
        )
        await callback.answer()
            ...
            await callback.message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="⬅️ Назад к просмотру", callback_data=f"back_to_view_after_error:{user_id}"
                            )
                        ],
                        [InlineKeyboardButton(text="≡ Главное меню", callback_data="main_menu")],
                    ]
...
async def callback_cancel_delete_user(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=get_user_editor_keyboard(is_trainee))
...
async def callback_cancel_edit(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(text, reply_markup=get_user_editor_keyboard(is_trainee), parse_mode="HTML")
