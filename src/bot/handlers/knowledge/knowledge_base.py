...
from bot.database.db import (
    ...
    get_folder_access_info,
    get_knowledge_folder_by_id,
    get_knowledge_material_by_id,
    get_user_by_tg_id,
    set_folder_access_groups,
    update_knowledge_folder_name,
)
from bot.database.models import KnowledgeMaterial
from bot.keyboards.keyboards import (
    get_employee_folder_materials_keyboard,
    get_employee_knowledge_folders_keyboard,
    get_employee_material_view_keyboard,
    get_folder_created_keyboard,
    get_folder_delete_confirmation_keyboard,
    get_folder_deleted_keyboard,
    get_folder_rename_confirmation_keyboard,
    get_folder_view_keyboard,
    get_group_access_selection_keyboard,
    ...
    get_material_delete_confirmation_keyboard,
    get_material_description_keyboard,
    get_material_save_keyboard,
    get_material_saved_keyboard,
    get_material_view_keyboard,
)
...
async def send_material_attachments(callback: CallbackQuery, material: KnowledgeMaterial, message_text: str) -> None:
    """Отправляет текст материала и вложения (превью-фото/документы, основной файл) в чат."""
    ...
    if material.material_type != "link":
        try:
            if material.material_type == "video":
                await callback.bot.send_video(chat_id=callback.message.chat.id, video=material.content)
            elif material.material_type == "photo":
                try:
                    await callback.bot.send_photo(chat_id=callback.message.chat.id, photo=material.content)
                except Exception as inner_error:
                    logger.error(
                        f"Ошибка отправки фото как photo для {material.name}: {inner_error}. Пробуем как document"
                    )
                    await callback.bot.send_document(chat_id=callback.message.chat.id, document=material.content)
            else:
                await callback.bot.send_document(chat_id=callback.message.chat.id, document=material.content)
        except Exception as file_error:
            logger.error(f"Ошибка отправки файла {material.name}: {file_error}")
            await callback.bot.send_message(
                chat_id=callback.message.chat.id, text=f"⚠️ Ошибка при отправке файла {material.name}."
            )
...
async def start_material_addition(
    callback: CallbackQuery, state: FSMContext, session: AsyncSession, folder_id: int
) -> bool:
    """
    ...
    try:
        ...
        await callback.message.edit_text(
            f"📚РЕДАКТОР БАЗЫ ЗНАНИЙ📚\n\n📁Папка: {folder.name}\n\n🟡Введи название материала:", parse_mode="HTML"
        )

        await state.set_state(KnowledgeBaseStates.waiting_for_material_name)
        log_user_action(
            callback.from_user.id, "material_creation_started", f"Начато создание материала в папке {folder.name}"
        )
        return True
...
async def callback_create_folder(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        # ТЗ 9-1 шаг 4
        await callback.message.edit_text(
            "📚РЕДАКТОР БАЗЫ ЗНАНИЙ📚\n\nВведи название для новой папки:", parse_mode="HTML"
        )
...
async def process_folder_name(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if not validate_name(folder_name):
            await message.answer(
                "❌ Некорректное название папки. Название должно содержать от 2 до 100 символов и состоять из букв, цифр и основных знаков препинания."
            )
            return
...
@router.callback_query(
    F.data.startswith("kb_add_material_to_folder:"),
    StateFilter(KnowledgeBaseStates.main_menu, KnowledgeBaseStates.viewing_folder),
)
...
async def process_material_name(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if not validate_name(material_name):
            await message.answer(
                "❌ Некорректное название материала. Название должно содержать от 2 до 100 символов и состоять из букв, цифр и основных знаков препинания."
            )
            return
...
async def process_material_content(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if message.document:
            ...
            await message.answer("❌ Отправь документ, изображение или ссылку")
            return

        # Обновляем состояние
        await state.update_data(material_content=material_content, material_type=material_type)
    ...
    try:
        ...
        if len(normalized) >= 10:
            await message.answer(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[[InlineKeyboardButton(text="✅Продолжить", callback_data="kb_finish_photos")]]
                ),
            )
            return
        ...
        log_user_action(
            ...
            "material_photos_added",
            f"Добавлено {len(photos)} фото к материалу, всего: {photos_count}",
        )
...
async def process_material_image_docs(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        log_user_action(
            message.from_user.id,
            "material_image_doc_added",
            f"Добавлен image-document к материалу, всего: {photos_count}",
        )
...
async def show_photo_upload_option(message_or_callback, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if hasattr(message_or_callback, "from_user") and message_or_callback.from_user:
            ...
            hasattr(message_or_callback, "message")
            and message_or_callback.message
            and message_or_callback.message.from_user
        ):
        ...
        log_user_action(
            ...
            "material_photo_upload_offered",
            f"Предложено добавить фото к материалу: {material_name}",
        )
        ...
        log_user_error(
            ...
            "show_photo_upload_option_error",
            str(e),
        )
    ...
    try:
        ...
        if hasattr(message_or_callback, "text") or hasattr(message_or_callback, "photo"):
            ...
            await message_or_callback.answer(
                confirmation_text, reply_markup=get_material_save_keyboard(), parse_mode="HTML"
            )
        else:
            # CallbackQuery
            await message_or_callback.message.edit_text(
    ...
    try:
        ...
        for i, mat in enumerate(active_materials, 1):
            ...
            materials_display.append(
                f"🔗Материал {i}: {mat.name}\n🟢Вложение: {mat_content}\n🟢 Описание: {mat_description}{photos_info}"
            )
        ...
        await callback.message.edit_text(
            final_message, reply_markup=get_material_saved_keyboard(folder_id), parse_mode="HTML"
        )
...
async def callback_view_material(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await send_material_attachments(callback, material, message_text)
                ...
                if photo_ids:
                    if len(photo_ids) == 1:
                        ...
                        for i, file_id in enumerate(photo_ids, 1):
                            if i == 1:
                                media_group.append(
                                    InputMediaPhoto(media=file_id, caption=message_text, parse_mode="HTML")
                                )
                            else:
                                media_group.append(InputMediaPhoto(media=file_id))
                        await callback.bot.send_media_group(chat_id=callback.message.chat.id, media=media_group)
...
@router.callback_query(
    F.data.startswith("kb_confirm_delete_material:"), StateFilter(KnowledgeBaseStates.confirming_material_deletion)
)
...
async def callback_folder_access(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await state.update_data(current_folder_id=folder_id, selected_group_ids=current_group_ids.copy())
...
async def callback_toggle_group_access(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        message_text = f"📚РЕДАКТОР БАЗЫ ЗНАНИЙ📚\n\n📁Папка: {folder.name}\n🔒Доступ: {access_text}\n\n"
...
async def callback_save_access(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        log_user_action(
            callback.from_user.id, "folder_access_saved", f"Сохранены настройки доступа для папки {folder_id}"
        )
...
async def callback_rename_folder(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        # ТЗ 9-4 шаг 5
        await callback.message.edit_text(
            f"📚РЕДАКТОР БАЗЫ ЗНАНИЙ📚\n\n📁Папка: {folder.name}\n🟡Введи новое название для папки:", parse_mode="HTML"
        )
...
async def process_new_folder_name(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if not validate_name(new_name):
            await message.answer(
                "❌ Некорректное новое название папки. Название должно содержать от 2 до 100 символов и состоять из букв, цифр и основных знаков препинания."
            )
            return
        ...
        # ТЗ 9-4 шаг 7
        await message.answer(
            f"📚РЕДАКТОР БАЗЫ ЗНАНИЙ📚\n\n📁Папка: {old_name}\n🟡Новое название для папки: {new_name}",
...
async def callback_confirm_rename(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        if not success:
            await callback.message.edit_text(
                "❌ Не удалось переименовать папку. Возможно, папка с таким названием уже существует."
            )
            return
...
@router.callback_query(
    F.data.startswith("kb_confirm_delete_folder:"), StateFilter(KnowledgeBaseStates.confirming_folder_deletion)
)
...
async def callback_employee_view_material(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        log_user_action(
            callback.from_user.id, "employee_material_viewed", f"Сотрудник просмотрел материал: {material.name}"
        )
...
# ===============================
# Поиск материалов по Базе Знаний
# ===============================
...
async def render_kb_search_results(
    ...
    """Поиск материалов и отрисовка результатов (новым сообщением или редактированием)."""
    company_id = await ensure_company_id(session, state, user_tg_id)
    if company_id is None:
        text = "❌ Не удалось определить компанию. Обнови сессию командой /start."
        if edit:
    ...
    if data.get("kb_search_mode") == "viewer":
        ...
        if not user:
            ...
            if edit:
                ...
                await target_message.answer(text)
            return
        folders = await get_accessible_knowledge_folders_for_user(session, user.id, company_id)
...
async def callback_kb_search(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    """Вход в поиск по БЗ из меню редактора или режима просмотра"""
    try:
        await callback.answer()
        ...
        log_user_action(callback.from_user.id, "kb_search_opened", f"Открыт поиск по БЗ ({mode})")

    except Exception as e:
        await callback.message.edit_text("Произошла ошибка при открытии поиска")
        log_user_error(callback.from_user.id, "kb_search_open_error", str(e))


@router.message(KnowledgeBaseStates.waiting_for_search_query)
async def process_kb_search_query(message: Message, state: FSMContext, session: AsyncSession):
    """Обработка поискового запроса по материалам БЗ"""
    try:
        ...
        await state.update_data(kb_search_query=query, kb_search_page=0)
        await render_kb_search_results(message, state, session, message.from_user.id, query, page=0, edit=False)

        log_user_action(message.from_user.id, "kb_search_performed", f"Поиск по БЗ: '{query}'")

    except Exception as e:
        await message.answer("Произошла ошибка при поиске материалов")
        log_user_error(message.from_user.id, "kb_search_query_error", str(e))


@router.callback_query(F.data.startswith("kb_search_page:"), StateFilter(KnowledgeBaseStates.viewing_search_results))
async def callback_kb_search_page(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    """Пагинация результатов поиска по БЗ"""
    try:
        await callback.answer()
        ...
        await callback.message.edit_text("Произошла ошибка при просмотре результатов")
        log_user_error(callback.from_user.id, "kb_search_page_error", str(e))


@router.callback_query(F.data.startswith("kb_search_result:"), StateFilter(KnowledgeBaseStates.viewing_search_results))
async def callback_kb_search_result(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    """Открытие материала из результатов поиска по БЗ"""
    try:
        await callback.answer()
        ...
        if (
            not material
            or not material.folder
            or not material.folder.is_active
            or material.folder.company_id != company_id
        ):
            await callback.message.edit_text(
                "❌ Материал не найден", reply_markup=get_kb_search_no_results_keyboard(), parse_mode="HTML"
            )
            await state.set_state(KnowledgeBaseStates.waiting_for_search_query)
            return
        ...
        if data.get("kb_search_mode") == "viewer":
            ...
            if not has_access:
                await callback.message.edit_text(
                    "❌ У тебя нет доступа к этому материалу.",
        ...
        await callback.bot.send_message(
            chat_id=callback.message.chat.id,
            text="📋 Навигация по результатам поиска",
            reply_markup=get_kb_search_material_view_keyboard(),
            parse_mode="HTML",
        )

        log_user_action(
            callback.from_user.id, "kb_search_material_viewed", f"Из поиска открыт материал: {material.name}"
        )

    except Exception as e:
        await callback.message.edit_text("Произошла ошибка при просмотре материала")
        log_user_error(callback.from_user.id, "kb_search_result_error", str(e))


@router.callback_query(F.data == "kb_search_back_to_results", StateFilter(KnowledgeBaseStates.viewing_search_results))
async def callback_kb_search_back_to_results(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    """Возврат от материала к результатам поиска"""
    try:
        await callback.answer()
        ...
        await callback.message.edit_text("Произошла ошибка при возврате к результатам")
...
async def callback_kb_search_cancel(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text("Произошла ошибка при возврате")
        log_user_error(callback.from_user.id, "kb_search_cancel_error", str(e))
