...
async def send_role_welcome(message: Message, user, primary_role: str):
    ...
    if primary_role in ("Наставник", "Стажер"):
        await message.answer(
            f"Добро пожаловать, {user.full_name}! Ты вошел как {primary_role}.", reply_markup=ReplyKeyboardRemove()
        )
    ...
    if photo_source:
        try:
            await message.answer_photo(
                photo=photo_source, caption=menu_text, parse_mode="HTML", reply_markup=get_mentor_inline_menu()
            )
            return
        except Exception:
            pass

    await message.answer(menu_text, parse_mode="HTML", reply_markup=get_mentor_inline_menu())
...
async def send_trainee_menu(message: Message):
    ...
    if photo_source:
        try:
            await message.answer_photo(
                photo=photo_source, caption=menu_text, parse_mode="HTML", reply_markup=get_trainee_inline_menu()
            )
            return
        except Exception as e:
            logger.warning(f"Не удалось отправить изображение меню стажера: {e}")

    await message.answer(menu_text, parse_mode="HTML", reply_markup=get_trainee_inline_menu())
