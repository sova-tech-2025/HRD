...
class TestFormatProfileText:
    ...
    async def test_basic_profile_fields(self):
        ...
        from bot.utils.messages.common import format_profile_text

        user = make_user()
        session = AsyncMock()

        with patch("bot.utils.messages.common.get_user_roles", return_value=[FakeRole("Сотрудник")]):
    ...
    async def test_trainee_shows_internship_object(self):
        ...
        with patch("bot.utils.messages.common.get_user_roles", return_value=[FakeRole("Стажер")]):
            result = await format_profile_text(user, session)
    ...
    async def test_role_displayed(self):
        ...
        with patch("bot.utils.messages.common.get_user_roles", return_value=[FakeRole("Наставник")]):
            result = await format_profile_text(user, session)
    ...
    async def test_multiple_roles_primary_displayed(self):
        ...
        with patch("bot.utils.messages.common.get_user_roles", return_value=roles):
            result = await format_profile_text(user, session)
