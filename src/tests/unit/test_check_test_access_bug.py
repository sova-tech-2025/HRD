...
class TestTraineeAccessWithTraineeTestAccess:
    ...
    async def test_trainee_access_granted_when_access_record_exists(self):
        ...
        """
        from bot.database.db import check_test_access
        ...
        with patch("bot.database.db.get_user_roles", return_value=[make_role("Стажер")]):
    ...
    async def test_trainee_access_with_null_company_and_valid_test(self):
        ...
        with (
            patch("bot.database.db.get_user_roles", return_value=[make_role("Стажер")]),
            patch("bot.database.db.get_test_by_id", return_value=test_mock),
        ):
            result = await check_test_access(session, user_id=568, test_id=16, company_id=1)
...
class TestEmployeeAccess:
    ...
    async def test_employee_access_to_non_recruiter_test(self):
        ...
        with (
            patch("bot.database.db.get_user_roles", side_effect=roles_side_effect),
            patch("bot.database.db.get_test_by_id", return_value=test_mock),
            patch("bot.database.db.get_user_by_id", return_value=creator_mock),
        ):
    ...
    async def test_employee_denied_without_access_record(self):
        ...
        with (
            patch("bot.database.db.get_user_roles", return_value=[make_role("Сотрудник")]),
            patch("bot.database.db.get_test_by_id", return_value=test_mock),
        ):
    ...
    async def test_employee_denied_nonexistent_test(self):
        ...
        with (
            patch("bot.database.db.get_user_roles", return_value=[make_role("Сотрудник")]),
            patch("bot.database.db.get_test_by_id", return_value=None),
        ):
...
class TestOtherRolesAccess:
    ...
    async def test_recruiter_access_same_company(self):
        ...
        with (
            patch("bot.database.db.get_user_roles", return_value=[make_role("Рекрутер")]),
            patch("bot.database.db.get_test_by_id", return_value=test_mock),
        ):
    ...
    async def test_mentor_denied_cross_company(self):
        ...
        with (
            patch("bot.database.db.get_user_roles", return_value=[make_role("Наставник")]),
            patch("bot.database.db.get_test_by_id", return_value=test_mock),
        ):
...
class TestEdgeCases:
    ...
    async def test_trainee_denied_when_no_company_id(self):
        ...
        with (
            patch("bot.database.db.get_user_roles", return_value=[make_role("Стажер")]),
            patch("bot.database.db.get_user_by_id", return_value=MagicMock(company_id=None)),
        ):
    ...
    async def test_exception_returns_false(self):
        ...
        with patch("bot.database.db.get_user_roles", side_effect=Exception("DB error")):
