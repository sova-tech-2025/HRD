...
class TestCallbackTraineeProfile:
    ...
    async def test_inactive_user_blocked(self):
        ...
        from handlers.core.common import callback_trainee_profile

        callback = make_callback()
        user = make_user(is_active=False)
        session = AsyncMock()

        with patch("handlers.core.common.get_validated_user", return_value=None) as mock_validate:
    ...
    async def test_user_not_found_blocked(self):
        ...
        with patch("handlers.core.common.get_validated_user", return_value=None):
            await callback_trainee_profile(callback, session)
    ...
    async def test_valid_user_sees_profile(self):
        ...
        with (
            patch("handlers.core.common.get_validated_user", return_value=user),
            patch("handlers.core.common.format_profile_text", return_value="<b>Профиль</b>") as mock_fmt,
        ):
            await callback_trainee_profile(callback, session)
...
class TestCallbackTraineeHelp:
    ...
    async def test_inactive_user_blocked(self):
        ...
        from handlers.core.common import callback_trainee_help
    ...
    async def test_valid_user_sees_help(self):
        ...
        with (
            patch("handlers.core.common.get_validated_user", return_value=user),
            patch("handlers.core.common.get_user_roles", return_value=roles),
            patch("handlers.core.common.format_help_message", return_value="Справка") as mock_help,
        ):
            await callback_trainee_help(callback, session)
