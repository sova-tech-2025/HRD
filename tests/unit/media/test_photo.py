...
class TestResolvePhoto:
    ...
    def test_file_id_has_highest_priority(self):
        from utils.media.photo import _resolve_photo
    ...
    def test_path_used_when_no_file_id_and_no_url(self):
        from utils.media.photo import _resolve_photo
        with patch("utils.media.photo.FSInputFile") as mock_fs:
    ...
    def test_path_error_returns_none(self):
        from utils.media.photo import _resolve_photo
        with patch("utils.media.photo.FSInputFile", side_effect=Exception("file not found")):
...
class TestMenuPhotoFunctions:
    ...
    def test_mentor_photo_uses_mentor_config(self):
        from utils.media.photo import get_mentor_menu_photo
        with patch("utils.media.photo.MENTOR_MENU_IMAGE_FILE_ID", "mentor_file_id"), \
             patch("utils.media.photo.MENTOR_MENU_IMAGE_PATH", "/mentor.jpg"):
            assert get_mentor_menu_photo() == "mentor_file_id"

    def test_mentor_photo_falls_back_to_path(self):
        from utils.media.photo import get_mentor_menu_photo
        with patch("utils.media.photo.MENTOR_MENU_IMAGE_FILE_ID", ""), \
             patch("utils.media.photo.MENTOR_MENU_IMAGE_PATH", "/mentor.jpg"), \
             patch("utils.media.photo.FSInputFile") as mock_fs:
            mock_fs.return_value = MagicMock(spec=FSInputFile)
    ...
    def test_mentor_photo_returns_none_when_not_configured(self):
        from utils.media.photo import get_mentor_menu_photo
        with patch("utils.media.photo.MENTOR_MENU_IMAGE_FILE_ID", ""), \
             patch("utils.media.photo.MENTOR_MENU_IMAGE_PATH", ""):
            assert get_mentor_menu_photo() is None

    def test_trainee_photo_uses_trainee_config(self):
        from utils.media.photo import get_trainee_menu_photo
        with patch("utils.media.photo.TRAINEE_MENU_IMAGE_FILE_ID", "trainee_file_id"), \
             patch("utils.media.photo.TRAINEE_MENU_IMAGE_PATH", "/trainee.jpg"):
            assert get_trainee_menu_photo() == "trainee_file_id"

    def test_trainee_photo_returns_none_when_not_configured(self):
        from utils.media.photo import get_trainee_menu_photo
        with patch("utils.media.photo.TRAINEE_MENU_IMAGE_FILE_ID", ""), \
             patch("utils.media.photo.TRAINEE_MENU_IMAGE_PATH", ""):
            assert get_trainee_menu_photo() is None

    def test_main_menu_photo_uses_file_id(self):
        from utils.media.photo import get_main_menu_photo
        with patch("utils.media.photo.MAIN_MENU_IMAGE_FILE_ID", "main_file_id"), \
             patch("utils.media.photo.MAIN_MENU_IMAGE_PATH", ""), \
             patch("utils.media.photo.MAIN_MENU_IMAGE_URL", ""):
            assert get_main_menu_photo() == "main_file_id"

    def test_main_menu_photo_falls_back_to_url(self):
        from utils.media.photo import get_main_menu_photo
        with patch("utils.media.photo.MAIN_MENU_IMAGE_FILE_ID", ""), \
             patch("utils.media.photo.MAIN_MENU_IMAGE_URL", "https://example.com/main.jpg"), \
             patch("utils.media.photo.MAIN_MENU_IMAGE_PATH", "/main.jpg"):
