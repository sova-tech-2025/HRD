...
class TestScenario5_BroadcastTestAccess:
    ...
    async def test_step1_recruiter_creates_broadcast(
        ...
        resp = await recruiter.send_and_wait(
            "Рассылка ✈️", pattern="[Рр]ассылк|действие|Создать"
        )

        # Нажимаем "Создать рассылку"
        resp = await recruiter.click_and_wait(
            resp, data=b"create_broadcast",
            wait_pattern="текст|сообщение|Введи"
        )

        # Вводим текст рассылки
        resp = await recruiter.send_and_wait(
            "E2E тестовая рассылка: проверьте свои знания!",
            pattern="фото|Фото|изображени|материал"
        )
        ...
        if skip_photo_btn:
            resp = await recruiter.click_and_wait(
                ...
                wait_pattern="материал|папк|тест|Выбери"
            )
        ...
        test_btn = recruiter.find_button_data(
            resp, text_contains="Кофе", data_prefix="broadcast_test:"
        )
        if not test_btn:
            # Может быть другой формат кнопки
            test_btn = recruiter.find_button_data(
                resp, text_contains="Кофе"
            )
        assert test_btn, (
        ...
        resp = await recruiter.click_and_wait(
            resp, data=test_btn,
            wait_pattern="рол[иеь]|Рол[иеь]|Кому|получател|Выбор рол"
        )
        ...
        if next_btn:
            resp = await recruiter.click_and_wait(
                resp, data=next_btn,
                wait_pattern="групп|Групп"
            )

        # Выбираем группу "Бариста"
        group_btn = recruiter.find_button_data(
            resp, text_contains="Бариста", data_prefix="broadcast_group:"
        )
        if group_btn:
    ...
    async def test_step3_employee_can_take_broadcast_test(
        ...
        resp = await trainee2.send_and_wait(
            "Мои тесты 📋", pattern="тест|Тест|Нет тестов"
        )
